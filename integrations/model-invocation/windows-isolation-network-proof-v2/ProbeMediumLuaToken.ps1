...
public sealed class MatawakaMediumLuaObservation
{
    ...
    public bool MediumIntegrityApplied { get; init; }
    public uint CanaryExitCode { get; init; }
}

public static class MatawakaMediumLuaProbe
{
    ...
    public static MatawakaMediumLuaObservation Run(string workingDirectory)
    {
        ...
        try
        {
            ...
            if (after != 0x2000) throw new InvalidOperationException($"LUA_MEDIUM_INTEGRITY_NOT_APPLIED:{after}");
            ...
            uint exitCode = RunCanary(lua, canary, workingDirectory);

            return new MatawakaMediumLuaObservation
            {
                ...
                MediumIntegrityApplied = after == 0x2000,
    ...
    private static uint RunCanary(IntPtr token, string executable, string workingDirectory)
    {
        ...
        if (!CreateProcessAsUser(token, executable, command, IntPtr.Zero, IntPtr.Zero, false, CREATE_NO_WINDOW,
            ...
            ThrowWin32("CREATE_MEDIUM_LUA_CANARY");
        try
        {
            uint wait = WaitForSingleObject(pi.hProcess, INFINITE);
            if (wait != WAIT_OBJECT_0) throw new Win32Exception(Marshal.GetLastWin32Error(), $"WAIT_MEDIUM_LUA_CANARY:{wait}");
            if (!GetExitCodeProcess(pi.hProcess, out uint exitCode)) ThrowWin32("GET_MEDIUM_LUA_CANARY_EXIT");
            return exitCode;
        }
        finally
        {
    ...
    private static bool QueryBoolean(IntPtr token, int informationClass)
    {
        ...
        try
        {
            ...
            if (!GetTokenInformation(token, informationClass, buffer, sizeof(int), out uint returned))
                ThrowWin32($"QUERY_BOOLEAN_{informationClass}");
    ...
    private static int QueryDword(IntPtr token, int informationClass)
    {
        ...
        {
            if (!GetTokenInformation(token, informationClass, buffer, sizeof(int), out uint returned))
                ThrowWin32($"QUERY_DWORD_{informationClass}");
...
$obs = [MatawakaMediumLuaProbe]::Run((Get-Location).Path)
$status = if ($obs.MediumIntegrityApplied -and $obs.CanaryExitCode -eq 0) { 'MEDIUM_LUA_CANARY_PASS' } else { 'MEDIUM_LUA_CANARY_FAIL_CLOSED' }
$record = [ordered]@{
    schema = 'matawaka.windows-medium-lua-parent-diagnostic/v0.1'
    status = $status
    ...
    luaElevationType = $obs.LuaElevationType
    ...
    mediumIntegrityApplied = $obs.MediumIntegrityApplied
    canaryExitCodeUnsigned = [uint32]$obs.CanaryExitCode
...
if ($status -ne 'MEDIUM_LUA_CANARY_PASS') { exit 3 }
exit 0
