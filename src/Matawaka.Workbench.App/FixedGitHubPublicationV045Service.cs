...
public sealed class FixedGitHubPublicationV045Service
{
    ...
    public const string AcceptedTag = "workbench-v0.45-accepted";
    public const string ExpectedParent = "2877b4bf95143780edff19231eedf57520bd57ec";
    public const string ExpectedParentTag = "workbench-v0.44.1-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.45";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.45";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.45 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.45 accepted HEAD parent is not exact accepted v0.44.1: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.44.1 predecessor tag does not point at exact v0.45 parent.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.45 tag does not point at current HEAD.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.45 contract.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact v0.44.1 parent nor exact v0.45 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.45 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main disappeared during v0.45 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main did not converge to exact accepted v0.45 HEAD.");
        if (tagState == "ABSENT")
        {
        ...
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main missing after v0.45 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag)
            ?? throw new InvalidDataException("Remote v0.45 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.45 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.45 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "legacy-control quarantine creates no publication or application mutation authority",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.45 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.45-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact v0.44.1 parent",
                "git push github-workbench refs/tags/workbench-v0.45-accepted:refs/tags/workbench-v0.45-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted v0.45 active-surface contract. Quarantining legacy compatibility controls does not expand authority.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.45-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v045-fixed-tag", AcceptedTag == "workbench-v0.45-accepted", AcceptedTag, "workbench-v0.45-accepted"),
        ("publisher-v045-fixed-parent", ExpectedParent == "2877b4bf95143780edff19231eedf57520bd57ec", ExpectedParent, "accepted v0.44.1"),
        ("publisher-v045-parent-tag", ExpectedParentTag == "workbench-v0.44.1-accepted", ExpectedParentTag, "workbench-v0.44.1-accepted"),
        ("publisher-v045-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v045-surface-not-authority", true, "explicit Publish button only", "separate from operator-surface quarantine")
    };

    public static string ClassifyRemoteMain(string parent, string head, string remote)
        => remote.Equals(parent, StringComparison.OrdinalIgnoreCase) ? "PARENT" :
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            !head.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) ||
            !tag.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            !parentTag.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.45 frontier changed after publication preview.");
    }
    ...
    private static Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, params string[] args)
        => RunGitAsync(root, cancellationToken, false, args);
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.45 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.45 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.45 publication Git operation failed: {stderr.Trim()}");
