...
/// <summary>
/// v0.14 consumes a READY bounded update plan only after explicit UI confirmation.
...
{
    public const string ReceiptSchema = "matawaka.workbench-update-materialization-receipt/v0.14";
    public const string AuthoritySchema = "matawaka.workbench-update-materialization-authority-receipt/v0.14";
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var receipt = new WorkbenchUpdateMaterializationReceipt(
                ...
                nonEffects,
                "v0.14 materialization remains a reversible staging effect only and carries its predecessor tag explicitly. The validated bytes are copied to an ignored Workbench-local staging area after explicit human confirmation. No build, tracked-source apply, Git checkpoint, network, catalog mutation, or Agent Execute authority is inferred.");

            var artifactDir = Path.Combine(repositoryRoot, "artifacts", "update-materializations");
            Directory.CreateDirectory(artifactDir);
            var artifactPath = Path.Combine(artifactDir, $"materialization-v0.14-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
