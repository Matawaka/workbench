...
public sealed class LocalCheckpointV039Service
{
    public const string Version = "0.39.0";
    public const string ExpectedPredecessorCommit = "612eccf4ff440b27f37fe634205980c3e2f9e0d8";
    public const string ExpectedPredecessorTag = "workbench-v0.38.1-accepted";
    public const string TargetTag = "workbench-v0.39-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.39 candidate launch handoff";
    ...
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != "matawaka.workbench-acceptance-receipt/v0.39")
            throw new InvalidDataException("A passing Workbench v0.39 Self-test receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.38.1 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.38.1 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.39 source changes to checkpoint.");
    ...
    public async Task<LocalCheckpointReceipt> AcceptAsync(LocalCheckpointCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.39 contract.");
        ...
        try
        {
            ...
            await RunGitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m",
                "Accepted Workbench v0.39: verified candidate launch handoff and predecessor self-close; no candidate acceptance/process-kill authority");
            tagged = true;

            if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.39 checkpoint.");
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.39",
                "human-operator-at-workbench-ui", "git.local-checkpoint.accept", root,
                "explicit v0.39 Accept confirmation after passing v0.39 Self-test",
                true, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.39-message>", "git tag -a workbench-v0.39-accepted" },
                nonEffects);
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.39", Version, DateTimeOffset.Now,
                ...
                "Local v0.39 candidate-launch handoff checkpoint only. Launch/handoff/self-close/publication/lifecycle remain separately bounded operations.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.39-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v039-fixed-predecessor", ExpectedPredecessorCommit == "612eccf4ff440b27f37fe634205980c3e2f9e0d8", ExpectedPredecessorCommit, "accepted v0.38.1"),
        ("checkpoint-v039-fixed-target-tag", TargetTag == "workbench-v0.39-accepted", TargetTag, "workbench-v0.39-accepted")
    };

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.39 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.39 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != "matawaka.workbench-acceptance-receipt/v0.39" || parsed.Version != Version ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.39 Self-test receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir) ? Directory.GetFiles(dir, "v0.39.0-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault() : null;
        if (path is null) throw new InvalidDataException("v0.39 build source manifest is missing.");
        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.39 build source manifest could not be parsed.");
        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.39" || manifest.Version != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.39 build source manifest identity/predecessor.");
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.39 Self-test receipt.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        using var process = new Process { StartInfo = psi }; if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.39 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested) { try { process.Kill(true); } catch { } throw new InvalidDataException("v0.39 checkpoint Git operation timed out."); }
        var stdout = await stdoutTask; var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.39 checkpoint Git operation failed: {stderr.Trim()}");
