...
/// Maintenance Update Orchestrator contract checks. Self-test never selects a
/// package, materializes, mutates source, builds, launches, checkpoints or publishes.
/// </summary>
public sealed class WorkbenchV033AcceptanceHarness
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var orchestratorChecks = MaintenanceUpdateOrchestratorService.RunOfflineContractChecks()
            ...
            .ToArray();
        var checks = predecessor.Checks.Concat(orchestratorChecks).ToArray();
        var passed = predecessor.Passed && orchestratorChecks.All(item => item.Passed);
        var nonEffects = predecessor.NonEffects.Concat(new[]
        {
            ...
            "v0.33 Self-test performs no remote publication",
            "orchestrator contract validation does not create Agent Execute or ActionPermit authority"
