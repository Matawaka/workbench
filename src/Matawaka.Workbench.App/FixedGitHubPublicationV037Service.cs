...
public sealed class FixedGitHubPublicationV037Service
{
    ...
    public const string AcceptedTag = "workbench-v0.37-accepted";
    public const string ExpectedParent = "8f8bac01661c0b5614422c3708f1afb78a483c8b";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.37";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.37";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        var status = await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all");
        if (!string.IsNullOrWhiteSpace(status.Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.37 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.37 accepted HEAD parent is not exact accepted v0.36: {parent}");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"Accepted tag {AcceptedTag} does not point at current HEAD.");
    ...
    {
        if (candidate is null || candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.37 contract.");
        ...
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main did not converge to exact accepted HEAD.");
        if (tagState == "ABSENT")
        {
        ...
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag) ?? throw new InvalidDataException("Remote accepted tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !remoteTagAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.37 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.37 publication.");
        ...
        var nonEffects = new[]
        {
            ...
            "no local-app package-build/update authority created by publication",
            "no catalog repository mutation",
            "no Agent Execute authority",
            "no ActionPermit creation",
            "no lifecycle action authority created by publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted button + separate confirmation dialog after local v0.37 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.37-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact parent",
                "git push github-workbench refs/tags/workbench-v0.37-accepted:refs/tags/workbench-v0.37-accepted when remote tag absent"
            },
            nonEffects);
        ...
        return new FixedGitHubPublicationReceipt(
            ...
            remoteAdded, mainPush, tagPush, recovery, remoteMainAfter, remoteTagAfter,
            true, true, authority, nonEffects,
            "Explicit human publication of one already-accepted Workbench v0.37 checkpoint. This does not create local-app package-build/update, lifecycle, Agent Execute or general network authority.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.37-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v037-fixed-tag", AcceptedTag == "workbench-v0.37-accepted", AcceptedTag, "workbench-v0.37-accepted"),
        ("publisher-v037-fixed-parent", ExpectedParent == "8f8bac01661c0b5614422c3708f1afb78a483c8b", ExpectedParent, "accepted v0.36"),
        ("publisher-v037-conflicting-main-refused", ClassifyRemoteMain("a", "b", "c") == "CONFLICT", ClassifyRemoteMain("a", "b", "c"), "CONFLICT"),
        ("publisher-v037-conflicting-tag-refused", ClassifyRemoteTag("b", "c") == "CONFLICT", ClassifyRemoteTag("b", "c"), "CONFLICT")
    };

    public static string ClassifyRemoteMain(string parent, string head, string remoteMain)
        => remoteMain.Equals(parent, StringComparison.OrdinalIgnoreCase) ? "PARENT" : remoteMain.Equals(head, StringComparison.OrdinalIgnoreCase) ? "ALREADY_HEAD" : "CONFLICT";
    public static string ClassifyRemoteTag(string head, string? remoteTag)
        => string.IsNullOrWhiteSpace(remoteTag) ? "ABSENT" : remoteTag.Equals(head, StringComparison.OrdinalIgnoreCase) ? "ALREADY_HEAD" : "CONFLICT";
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) || !head.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) || !tagHead.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted Workbench v0.37 frontier changed after publication preview.");
    }
    ...
    private static async Task<bool> EnsureFixedRemoteAsync(string root, CancellationToken cancellationToken)
    {
        ...
        if (!SameRemoteUrl(current, RemoteUrl)) throw new InvalidDataException($"Remote {RemoteName} conflicts with fixed URL: {current}");
        return false;
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.37 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.37 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.37 publication Git operation failed: {stderr.Trim()}");
