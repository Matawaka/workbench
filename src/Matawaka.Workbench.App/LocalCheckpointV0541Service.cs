...
public sealed record LocalCheckpointCandidateV0541(
    string Version,
...
public sealed record LocalCheckpointReceiptV0541(
    string Schema,
    string Version,
...
public sealed class LocalCheckpointV0541Service
{
    public const string Version = "0.54.1";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.54.1";
    public const string ExpectedPredecessorTag = "workbench-v0.54-accepted";
    public const string ExpectedPredecessorCommit = "b501ba5820d0ae03265723b8d8cd413ba4818984";
    public const string TargetTag = "workbench-v0.54.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.54.1 acquisition receipt status binding correction";
    // The accepted v0.54 updater emits target 0.54.1 in schema family major.minor = v0.54.
    public const string BuildManifestSchema = "matawaka.workbench-build-source-manifest/v0.54";
    public const string BuildManifestPattern = "v0.54.1-source-manifest*.json";

    private static readonly TimeSpan GitTimeout = TimeSpan.FromSeconds(30);
    private static readonly JsonSerializerOptions JsonOptions = new() { PropertyNameCaseInsensitive = true, WriteIndented = true };

    public async Task<LocalCheckpointCandidateV0541> PreviewAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.54.1 exact local predecessor mismatch: expected {ExpectedPredecessorCommit}, observed {head}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.54-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.54.1 source changes to checkpoint.");
        ...
        return new LocalCheckpointCandidateV0541(
    ...
    public async Task<LocalCheckpointReceiptV0541> AcceptFromBootstrapAsync(
        LocalCheckpointCandidateV0541 candidate,
        string bootstrapLeaseId,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.54.1 local acceptance.");
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.54.1 contract.");
        ...
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase) || !head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.54.1 checkpoint preview.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted local v0.54 predecessor tag moved after preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.54.1 checkpoint preview.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.54.1 acceptance artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            if (!newParent.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.54.1 checkpoint parent is not the exact local v0.54 accepted predecessor.");
            await GitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.54.1: acquisition receipt status binding correction");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.54.1 checkpoint.");

            var nonEffects = NonEffects();
            return new LocalCheckpointReceiptV0541(
                "matawaka.workbench-local-checkpoint-receipt/v0.54.1", Version, DateTimeOffset.Now,
                ...
                $"Local v0.54.1 compatibility checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}; exact predecessor was {ExpectedPredecessorCommit} / {ExpectedPredecessorTag}.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceiptV0541 receipt, CancellationToken cancellationToken)
    {
        var dir = Path.Combine(ResolveRepositoryRoot(workspaceRoot), "artifacts", "acceptance");
        Directory.CreateDirectory(dir);
        var path = Path.Combine(dir, $"checkpoint-v0.54.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v0541-version", Version == "0.54.1", Version, "0.54.1"),
        ("checkpoint-v0541-predecessor", ExpectedPredecessorCommit == "b501ba5820d0ae03265723b8d8cd413ba4818984", ExpectedPredecessorCommit, "exact accepted local v0.54"),
        ("checkpoint-v0541-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.54-accepted", ExpectedPredecessorTag, "workbench-v0.54-accepted"),
        ("checkpoint-v0541-target-tag", TargetTag == "workbench-v0.54.1-accepted", TargetTag, "workbench-v0.54.1-accepted"),
        ("checkpoint-v0541-build-manifest", BuildManifestSchema == "matawaka.workbench-build-source-manifest/v0.54" && BuildManifestPattern == "v0.54.1-source-manifest*.json", BuildManifestSchema, "predecessor writer major.minor schema + exact target filename"),
        ("checkpoint-v0541-publication", true, "RemotePushAllowed=false", "publication remains separate after real-host smoke")
    };

    private static IReadOnlyList<string> NonEffects() => new[]
    {
        "exact predecessor is workbench-v0.54-accepted at b501ba5820d0ae03265723b8d8cd413ba4818984",
        "correction changes only canonical v0.52 execution-receipt status binding and first-boot checkpoint plumbing",
        "v0.54 materialization authority/state/schema remain v0.54",
        "no remote publication during local checkpoint",
        "no network acquisition, extraction, process start/stop, benchmark/model/game authority",
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.54.1 acceptance receipt is required.");
    }

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.54.1 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.54.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.54.1 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ...
            : null;
        if (path is null) throw new InvalidDataException("v0.54.1 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema || obj.GetProperty("Version").GetString() != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.54.1 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.54.1 build source manifest.");
        foreach (var item in bound)
        {
            ...
            if (!File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Build-bound v0.54.1 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.54.1 acceptance receipt.");
    }
    ...
    private static string RequireSha(string value, string role)
    {
        ...
        if (text.Length != 40 || text.Any(ch => !Uri.IsHexDigit(ch))) throw new InvalidDataException($"Invalid Git SHA for {role}.");
        return text;
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        using var process = Process.Start(psi) ?? throw new InvalidOperationException("Failed to start fixed local git process.");
        var stdoutTask = process.StandardOutput.ReadToEndAsync(linked.Token);
        var stderrTask = process.StandardError.ReadToEndAsync(linked.Token);
        await process.WaitForExitAsync(linked.Token);
        var result = new GitResult(process.ExitCode, await stdoutTask, await stderrTask);
        if (!allowFailure && result.ExitCode != 0)
            throw new InvalidDataException($"Fixed local git operation failed: git {string.Join(' ', args)} :: {result.Stderr.Trim()}");
        return result;
    }
