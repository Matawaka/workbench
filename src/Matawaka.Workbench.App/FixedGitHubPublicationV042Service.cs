...
public sealed class FixedGitHubPublicationV042Service
{
    ...
    public const string AcceptedTag = "workbench-v0.42-accepted";
    public const string ExpectedParent = "4eb94de604de60909058b51dc3e0a1dc06fd9984";
    public const string ExpectedParentTag = "workbench-v0.41.2-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.42";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.42";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.42 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.42 accepted HEAD parent is not exact accepted v0.41.2: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.41.2 predecessor tag does not point at exact v0.42 parent.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.42 tag does not point at current HEAD.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.42 contract.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact v0.41.2 parent nor exact v0.42 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.42 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main disappeared during v0.42 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main did not converge to exact accepted v0.42 HEAD.");
        if (tagState == "ABSENT")
        {
        ...
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main missing after v0.42 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag)
            ?? throw new InvalidDataException("Remote v0.42 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.42 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.42 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "manual Self-test/Accept/Stop surface removal creates no publication authority",
            "installed Apps observation creates no registration/update/launch authority",
            "status color/progress overlay creates no authority",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.42 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.42-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact v0.41.2 parent",
                "git push github-workbench refs/tags/workbench-v0.42-accepted:refs/tags/workbench-v0.42-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted v0.42 compact operator surface. UI simplification and read-only app/status presentation do not extend publication authority.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.42-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v042-fixed-tag", AcceptedTag == "workbench-v0.42-accepted", AcceptedTag, "workbench-v0.42-accepted"),
        ("publisher-v042-fixed-parent", ExpectedParent == "4eb94de604de60909058b51dc3e0a1dc06fd9984", ExpectedParent, "accepted v0.41.2"),
        ("publisher-v042-parent-tag", ExpectedParentTag == "workbench-v0.41.2-accepted", ExpectedParentTag, "workbench-v0.41.2-accepted"),
        ("publisher-v042-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v042-ui-presentation-not-authority", true, "explicit Publish button only", "separate from shell presentation")
    };
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            ...
            throw new InvalidDataException("Local accepted v0.42 frontier changed after publication preview.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.42 publication Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.42 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.42 publication Git operation failed: {stderr.Trim()}");
