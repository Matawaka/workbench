...
public sealed class LocalCheckpointV051Service
{
    public const string Version = "0.51.0";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.51";
    public const string ExpectedPredecessorCommit = "632ddbb73e8d70b485f02d21f772674d429adf8c";
    public const string ExpectedPredecessorTag = "workbench-v0.50.2-accepted";
    public const string TargetTag = "workbench-v0.51-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.51 lease-gated live app browse surface";
    ...
    public async Task<LocalCheckpointCandidate> PreviewAsync(string workspaceRoot, string acceptanceArtifactPath, WorkbenchAcceptanceReceipt acceptance, CancellationToken cancellationToken)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.51 first-boot validation receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.50.2 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.50.2 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.51 source changes to checkpoint.");
    ...
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId)) throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.51 local acceptance.");
    ...
    private async Task<LocalCheckpointReceipt> AcceptCoreAsync(LocalCheckpointCandidate candidate, string bootstrapLeaseId, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || !candidate.PreviousHead.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase) || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.51 contract.");
        var root = candidate.RepositoryRoot;
        var head = RequireSha((await GitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Workbench HEAD changed after v0.51 checkpoint preview.");
        var predecessorTagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.50.2 predecessor tag moved after v0.51 preview.");
        var status = ParseStatusPaths((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal)) throw new InvalidDataException("Workbench working tree changed after v0.51 checkpoint preview.");
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.51 validation artifact disappeared before checkpoint.");
        if (!parsed.Passed || parsed.Version != Version || parsed.Schema != AcceptanceSchema) throw new InvalidDataException("v0.51 automatic local acceptance requires a still-passing exact validation artifact.");
        ...
        try
        {
            ...
            if (!newParent.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("v0.51 checkpoint parent is not exact accepted v0.50.2 predecessor.");
            await GitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.51: lease-gated live app browse surface");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout)) throw new InvalidDataException("Workbench working tree is not clean after v0.51 checkpoint.");
            var nonEffects = new[]
            {
                "no remote publication during local checkpoint",
                "no read lease, MCP adapter, Secure MCP Tunnel or ChatGPT plugin operation",
                "no private Apps/AppSources bytes committed",
                "no lease bearer, tunnel credential, local endpoint or browse runtime state committed",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.51",
                "human-operator-via-one-shot-transition-bootstrap", "git.local-checkpoint.accept", root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.51 validation Passed=true required",
                false, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.51-message>", "git tag -a workbench-v0.51-accepted" }, nonEffects);
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.51", Version, DateTimeOffset.Now,
                ...
                $"Local v0.51 checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}. Browse/read/tunnel/private app runtime state remains outside checkpoint payload.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.51-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v051-fixed-predecessor", ExpectedPredecessorCommit == "632ddbb73e8d70b485f02d21f772674d429adf8c", ExpectedPredecessorCommit, "accepted v0.50.2"),
        ("checkpoint-v051-fixed-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.50.2-accepted", ExpectedPredecessorTag, "workbench-v0.50.2-accepted"),
        ("checkpoint-v051-fixed-target-tag", TargetTag == "workbench-v0.51-accepted", TargetTag, "workbench-v0.51-accepted"),
        ("checkpoint-v051-bootstrap-only", true, "AcceptFromBootstrapAsync only", "no manual visible Accept path"),
        ("checkpoint-v051-runtime-secrets-not-git", true, "artifacts/.workbench/Tools forbidden checkpoint path", "not checkpoint payload")
    };

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.51 validation artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions) ?? throw new InvalidDataException("v0.51 validation artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId || !parsed.AppExecutableSha256.Equals(acceptance.AppExecutableSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Validation artifact does not match the passing in-memory v0.51 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        foreach (var item in bound)
        {
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException($"Build-bound v0.51 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Running executable digest does not match v0.51 validation receipt.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        using var process = new Process { StartInfo = psi }; if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.51 checkpoint Git process.");
        ...
        var o = await stdout; var e = await stderr; if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.51 checkpoint Git operation failed: {e.Trim()}"); return new GitResult(process.ExitCode, o, e);
    }
