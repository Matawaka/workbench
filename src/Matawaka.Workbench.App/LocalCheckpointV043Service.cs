...
/// <summary>
/// v0.43 local checkpoint is reachable only from the one-shot transition bootstrap.
/// Application tree tabs are observation-only and create no manual acceptance path.
/// </summary>
public sealed class LocalCheckpointV043Service
{
    public const string Version = "0.43.0";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.43";
    public const string ExpectedPredecessorCommit = "2a68f43b7c8b3a5e8aae7d960f2d997ffa7ec525";
    public const string ExpectedPredecessorTag = "workbench-v0.42-accepted";
    public const string TargetTag = "workbench-v0.43-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.43 clickable app tree tabs";
        ...
        CancellationToken cancellationToken)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing Workbench v0.43 first-boot validation receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.42 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.42 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.43 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.43 local acceptance.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.43 contract.");

        var root = candidate.RepositoryRoot;
        var head = RequireSha((await RunGitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.43 checkpoint preview.");
        var predecessorTagHead = RequireSha((await RunGitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.42 predecessor tag moved after v0.43 checkpoint preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.43 checkpoint preview.");
        ...
        var acceptance = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.43 validation artifact disappeared before checkpoint.");
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("v0.43 automatic local acceptance requires a still-passing exact validation artifact.");
        VerifyRunningExecutable(acceptance.AppExecutableSha256);
        ...
        try
        {
            ...
            if (!newParent.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.43 checkpoint parent is not exact accepted v0.42 predecessor.");
            await RunGitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m",
                "Accepted Workbench v0.43: clickable installed Apps with read-only tree tabs");
            tagged = true;

            if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.43 checkpoint.");

            var nonEffects = new[]
            {
                "app tree tabs observe names/type/size only and create no application authority",
                ...
                "no file-content inspection authority from tree view",
            ...
            {
                "git add -A -- .",
                "git commit -m <fixed-v0.43-message>",
                "git tag -a workbench-v0.43-accepted"
            };
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.43",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; originally authorized by explicit Update Workbench confirmation; exact v0.43 validation Passed=true required",
                false,
                true,
            ...
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.43", Version, DateTimeOffset.Now,
                ...
                $"Local v0.43 checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId} only after exact first-boot validation Passed=true. Clickable app tree tabs remain read-only observation.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.43-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v043-fixed-predecessor", ExpectedPredecessorCommit == "2a68f43b7c8b3a5e8aae7d960f2d997ffa7ec525", ExpectedPredecessorCommit, "accepted v0.42"),
        ("checkpoint-v043-fixed-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.42-accepted", ExpectedPredecessorTag, "workbench-v0.42-accepted"),
        ("checkpoint-v043-fixed-target-tag", TargetTag == "workbench-v0.43-accepted", TargetTag, "workbench-v0.43-accepted"),
        ("checkpoint-v043-bootstrap-only", true, "AcceptFromBootstrapAsync only", "no manual visible Accept path"),
        ("checkpoint-v043-publish-separate", true, "no push/fetch in allowed Git operations", "separate")
    };

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.43 validation artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.43 validation artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version ||
            ...
            throw new InvalidDataException("Validation artifact does not match the passing in-memory v0.43 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, "v0.43.0-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.43 build source manifest is missing.");
        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.43 build source manifest could not be parsed.");
        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.43" || manifest.Version != Version ||
            !manifest.PredecessorGitSha.Equals(predecessor, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Unexpected v0.43 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.43 build source manifest.");
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.43 validation receipt.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.43 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.43 checkpoint Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.43 checkpoint Git operation failed: {stderr.Trim()}");
