...
public sealed class FixedGitHubPublicationV047Service
{
    ...
    public const string AcceptedTag = "workbench-v0.47-accepted";
    public const string ExpectedParent = "8ca8a66f0892050e02024880a8d4bf4a6a8ec4d2";
    public const string ExpectedParentTag = "workbench-v0.46-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.47";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.47";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.47 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException($"v0.47 accepted HEAD parent is not exact accepted v0.46: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.46 predecessor tag does not point at exact v0.47 parent.");
        var tagHead = RequireSha((await RunGitAsync(root, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.47 tag does not point at current HEAD.");
    ...
    public async Task<FixedGitHubPublicationReceipt> PublishAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName || !SameRemoteUrl(candidate.RemoteUrl, RemoteUrl) || candidate.AcceptedTag != AcceptedTag || !candidate.Parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Publication candidate does not match fixed v0.47 contract.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact v0.46 parent nor exact v0.47 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.47 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main disappeared during v0.47 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Remote main did not converge to exact accepted v0.47 HEAD.");
        if (tagState == "ABSENT")
        {
            await RunGitAsync(root, cancellationToken, "push", RemoteName, $"refs/tags/{AcceptedTag}:refs/tags/{AcceptedTag}");
            tagPush = true;
        }
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main missing after v0.47 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag) ?? throw new InvalidDataException("Remote v0.47 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !remoteTagAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.47 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout)) throw new InvalidDataException("Local Workbench state changed during v0.47 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "no Apps/AppSources/private-context/chat-read clipboard bytes published",
            "no application update/launch/read/clipboard authority",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.47 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.47-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact v0.46 parent",
                "git push github-workbench refs/tags/workbench-v0.47-accepted:refs/tags/workbench-v0.47-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted Workbench v0.47 source only. Local clipboard/private app/source bytes are outside this Git repository and are not published.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.47-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v047-fixed-tag", AcceptedTag == "workbench-v0.47-accepted", AcceptedTag, "workbench-v0.47-accepted"),
        ("publisher-v047-fixed-parent", ExpectedParent == "8ca8a66f0892050e02024880a8d4bf4a6a8ec4d2", ExpectedParent, "accepted v0.46"),
        ("publisher-v047-parent-tag", ExpectedParentTag == "workbench-v0.46-accepted", ExpectedParentTag, "workbench-v0.46-accepted"),
        ("publisher-v047-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v047-clipboard-bytes", true, "outside Workbench repo", "not published")
    };

    public static string ClassifyRemoteMain(string parent, string head, string remote)
        => remote.Equals(parent, StringComparison.OrdinalIgnoreCase) ? "PARENT" : remote.Equals(head, StringComparison.OrdinalIgnoreCase) ? "ALREADY_HEAD" : "CONFLICT";
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        var parent = RequireSha((await RunGitAsync(candidate.RepositoryRoot, cancellationToken, "rev-parse", "HEAD^")).Stdout, "parent");
        var tag = RequireSha((await RunGitAsync(candidate.RepositoryRoot, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        var parentTag = RequireSha((await RunGitAsync(candidate.RepositoryRoot, cancellationToken, "rev-list", "-n", "1", ExpectedParentTag)).Stdout, ExpectedParentTag);
        if (!string.IsNullOrWhiteSpace(status.Stdout) || !head.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) || !tag.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !parentTag.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.47 frontier changed after publication preview.");
    }

    private static async Task<bool> EnsureFixedRemoteAsync(string root, CancellationToken cancellationToken)
    {
        var current = await GetConfiguredRemoteUrlAsync(root, cancellationToken);
        if (string.IsNullOrWhiteSpace(current))
        {
            await RunGitAsync(root, cancellationToken, "remote", "add", RemoteName, RemoteUrl);
            return true;
        }
    ...
    private static async Task<string?> GetConfiguredRemoteUrlAsync(string root, CancellationToken cancellationToken)
    {
        var result = await RunGitAsync(root, cancellationToken, true, "remote", "get-url", RemoteName);
        return result.ExitCode == 0 ? result.Stdout.Trim() : null;
    }

    private static async Task<string?> ReadRemoteMainAsync(string root, CancellationToken cancellationToken)
    {
        var result = await RunGitAsync(root, cancellationToken, "ls-remote", RemoteName, "refs/heads/main");
        var line = SplitLines(result.Stdout).SingleOrDefault();
        return line is null ? null : ParseLsRemoteSha(line);
    }

    private static async Task<string?> ReadRemoteTagCommitAsync(string root, CancellationToken cancellationToken, string tag)
    {
        var result = await RunGitAsync(root, cancellationToken, "ls-remote", RemoteName, $"refs/tags/{tag}", $"refs/tags/{tag}^{{}}");
    ...
    private static Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, params string[] args) => RunGitAsync(root, cancellationToken, false, args);

    private static async Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.47 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.47 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.47 publication Git operation failed: {stderr.Trim()}");
