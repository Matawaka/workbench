...
/// <summary>
/// Patch-level local acceptance successor over exact accepted v0.34. It exists
/// only to exercise the qualification/stabilization transition. Remote
/// publication and lifecycle assessment remain separate later decisions.
/// </summary>
public sealed class LocalCheckpointV0341Service
{
    public const string Version = "0.34.1";
    public const string ExpectedPredecessorCommit = "224ad00bd72b0534de1081b2a20c44746ee0e7a0";
    public const string ExpectedPredecessorTag = "workbench-v0.34-accepted";
    public const string TargetTag = "workbench-v0.34.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.34.1 qualification and stabilization";
        ...
        CancellationToken cancellationToken)
    {
        if (!acceptance.Passed ||
            ...
            !string.Equals(acceptance.Schema, "matawaka.workbench-acceptance-receipt/v0.34.1", StringComparison.Ordinal))
            throw new InvalidDataException("A passing Workbench v0.34.1 Self-test receipt is required before local stabilization checkpoint acceptance.");
        ...
        if (!string.Equals(currentHead, ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {currentHead} is not exact accepted v0.34 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (changedFiles.Count == 0)
            throw new InvalidDataException("There are no Workbench v0.34.1 stabilization source changes to checkpoint.");
        if (changedFiles.Any(IsForbiddenCheckpointPath))
            throw new InvalidDataException("Stabilization checkpoint contains a forbidden artifacts/.workbench path.");
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match the fixed v0.34.1 stabilization contract.");
        ...
        if (!string.Equals(headBefore, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.34.1 checkpoint preview.");

        var currentStatus = ParseStatusPaths(
            (await RunGitAsync(repositoryRoot, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (!currentStatus.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.34.1 checkpoint preview.");
        ...
        try
        {
            ...
            if (string.Equals(newHead, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Git commit did not advance v0.34.1 HEAD.");
            ...
            await RunGitAsync(
                ...
                "tag", "-a", TargetTag,
                "-m",
                "Accepted Workbench v0.34.1: qualification/stabilization patch makes lifecycle evidence routing successor-generic and public docs lifecycle-state-neutral; no new feature authority");
            tagCreated = true;
            ...
            if (!string.IsNullOrWhiteSpace(cleanStatus))
                throw new InvalidDataException("Workbench working tree is not clean after v0.34.1 checkpoint commit.");
            ...
            var nonEffects = new[]
            {
                ...
                "no lifecycle authority inferred from generic lifecycle adapter",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.34.1",
                "human-operator-at-workbench-ui",
                "git.local-checkpoint.accept",
                repositoryRoot,
                "explicit v0.34.1 Accept checkpoint button + confirmation after passing v0.34.1 Self-test",
                true,
                true,
                false,
                false,
                ...
                {
                    "git add -A -- .",
                    "git commit -m <fixed-v0.34.1-message>",
                    "git tag -a workbench-v0.34.1-accepted"
                },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.34.1",
                Version,
                DateTimeOffset.Now,
                ...
                true,
                nonEffects,
                "Local Workbench v0.34.1 qualification/stabilization acceptance only. Remote publication and successor-generic lifecycle assessment remain two separate later decisions.");
        }
        catch
        {
            if (tagCreated)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.34.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
        ...
        WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.34.1 acceptance artifact file is missing.");
        var full = Path.GetFullPath(artifactPath);
        var allowedRoot = Path.GetFullPath(Path.Combine(repositoryRoot, "artifacts", "acceptance")) +
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            ...
            ?? throw new InvalidDataException("v0.34.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed ||
            !string.Equals(parsed.Schema, "matawaka.workbench-acceptance-receipt/v0.34.1", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.34.1 Self-test receipt.");
        return full;
    }
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(
                    checkpointDir,
                    "v0.34.1-source-manifest*.json",
                    SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.34.1 build source manifest is missing. Refusing source not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(
            File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.34.1 build source manifest could not be parsed.");
        // BoundedUpdateApplyBuildService intentionally uses major.minor schema identity
        // while preserving the full patch version in Version and filename.
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.34", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Unexpected v0.34.1 build source manifest identity/predecessor.");
        ...
        if (!manifestFiles.Select(item => item.Path).SequenceEqual(currentFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Current changed-file set differs from the v0.34.1 source set captured at build time.");

        foreach (var item in manifestFiles)
        {
            var full = Path.GetFullPath(
                Path.Combine(repositoryRoot, item.Path.Replace('/', Path.DirectorySeparatorChar)));
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full))
                throw new InvalidDataException($"Build-bound v0.34.1 source file missing or escapes Workbench root: {item.Path}");
            if (!string.Equals(HashFile(full), item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"v0.34.1 source file changed after candidate build: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expectedSha256)
    {
        ...
        if (!string.Equals(HashFile(path), expectedSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest no longer matches v0.34.1 Self-test receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start())
            throw new InvalidDataException("Failed to start fixed v0.34.1 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.34.1 checkpoint Git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.34.1 checkpoint Git operation failed: {stderr.Trim()}");
