...
public sealed class FixedGitHubPublicationV0391Service
{
    ...
    public const string AcceptedTag = "workbench-v0.39.1-accepted";
    public const string ExpectedParent = "13f8618c6862b58a9e9de8772c69365058f34e91";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.39.1";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.39.1";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.39.1 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.39.1 accepted HEAD parent is not exact accepted v0.39: {parent}");
        var tagHead = RequireSha((await RunGitAsync(root, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.39.1 tag does not point at current HEAD.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.39.1 contract.");
        ...
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag)
            ?? throw new InvalidDataException("Remote tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.39.1 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "no candidate launch/handoff/predecessor-close authority created by publication",
            "no external process termination/signal authority",
            "no candidate acceptance authority created by publication",
            "no Local Apps package-build/update authority created by publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after local v0.39.1 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.39.1-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact parent",
                "git push github-workbench refs/tags/workbench-v0.39.1-accepted:refs/tags/workbench-v0.39.1-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted v0.39.1 handoff activation-probe successor. No launch/handoff/self-close/candidate-acceptance/lifecycle/Agent Execute/general network authority is created.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.39.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v0391-fixed-tag", AcceptedTag == "workbench-v0.39.1-accepted", AcceptedTag, "workbench-v0.39.1-accepted"),
        ("publisher-v0391-fixed-parent", ExpectedParent == "13f8618c6862b58a9e9de8772c69365058f34e91", ExpectedParent, "accepted v0.39"),
        ("publisher-v0391-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT")
    };
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            ...
            throw new InvalidDataException("Local accepted v0.39.1 frontier changed after publication preview.");
    }
    ...
    {
        var r = await RunGitAsync(root, cancellationToken: ct, allowFailure: false, "ls-remote", RemoteName, $"refs/tags/{tag}", $"refs/tags/{tag}^{{}}");
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start v0.39.1 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.39.1 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.39.1 publication Git operation failed: {stderr.Trim()}");
