...
public sealed class LocalCheckpointV0381Service
{
    public const string Version = "0.38.1";
    public const string ExpectedPredecessorCommit = "7c749b357f5c4095fe968f3fa18dba3cd1b52339";
    public const string ExpectedPredecessorTag = "workbench-v0.38-accepted";
    public const string TargetTag = "workbench-v0.38.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.38.1 explicit chooser layout stabilization";
    ...
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != "matawaka.workbench-acceptance-receipt/v0.38.1")
            throw new InvalidDataException("A passing Workbench v0.38.1 Self-test receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.38 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.38 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.38.1 source changes to checkpoint.");
    ...
    public async Task<LocalCheckpointReceipt> AcceptAsync(LocalCheckpointCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.38.1 contract.");
        ...
        try
        {
            ...
            await RunGitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m",
                "Accepted Workbench v0.38.1: chooser content-height layout stabilization; no package/update/launch authority expansion");
            tagged = true;

            if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.38.1 checkpoint.");
            ...
            var nonEffects = new[]
            {
                ...
                "no package-build receipt mutation performed by checkpoint",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.38.1",
                "human-operator-at-workbench-ui", "git.local-checkpoint.accept", root,
                "explicit v0.38.1 Accept confirmation after passing v0.38.1 Self-test",
                true, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.38.1-message>", "git tag -a workbench-v0.38.1-accepted" },
                nonEffects);
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.38.1", Version, DateTimeOffset.Now,
                ...
                "Local v0.38.1 chooser-layout stabilization checkpoint only. Local Apps/package build/update/publication/lifecycle remain separate authority decisions.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.38.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.38.1 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.38.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != "matawaka.workbench-acceptance-receipt/v0.38.1" || parsed.Version != Version ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.38.1 Self-test receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir) ? Directory.GetFiles(dir, "v0.38.1-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault() : null;
        if (path is null) throw new InvalidDataException("v0.38.1 build source manifest is missing.");
        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.38.1 build source manifest could not be parsed.");
        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.38" || manifest.Version != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.38.1 build source manifest identity/predecessor.");
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.38.1 Self-test receipt.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        using var process = new Process { StartInfo = psi }; if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.38.1 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested) { try { process.Kill(true); } catch { } throw new InvalidDataException("v0.38.1 checkpoint Git operation timed out."); }
        var stdout = await stdoutTask; var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.38.1 checkpoint Git operation failed: {stderr.Trim()}");
