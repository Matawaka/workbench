...
public sealed class FixedGitHubPublicationV0501Service
{
    ...
    public const string AcceptedTag = "workbench-v0.50.1-accepted";
    public const string ExpectedParent = "bd7b0c4cb69deaee2f575961b9f277e7168485c3";
    public const string ExpectedParentTag = "workbench-v0.50-accepted";
    public const string ExpectedRemoteBase = "1e0453ccf047bd948e76b577c2395a7d0009ff7a";
    public const string ExpectedRemoteBaseTag = "workbench-v0.49.1-accepted";
    public const string FailedRemoteTag = "workbench-v0.50-accepted";
    public const string HistoricalFailedRemoteTag = "workbench-v0.49-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.50.1";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.50.1";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.50.1 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.50.1 accepted HEAD parent is not exact operator-bound failed-v0.50 predecessor: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Local v0.50 predecessor tag does not point at exact v0.50.1 parent.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.50.1 tag does not point at current HEAD.");
    ...
    public async Task<FixedGitHubPublicationReceipt> PublishAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName || !SameRemoteUrl(candidate.RemoteUrl, RemoteUrl) || candidate.AcceptedTag != AcceptedTag || !candidate.Parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Publication candidate does not match fixed v0.50.1 stabilization contract.");
        ...
        if (await ReadRemoteTagCommitAsync(root, cancellationToken, FailedRemoteTag) is not null) throw new InvalidDataException("Failed real-host v0.50 tag unexpectedly exists remotely; v0.50.1 publication refused.");
        if (await ReadRemoteTagCommitAsync(root, cancellationToken, HistoricalFailedRemoteTag) is not null) throw new InvalidDataException("Historical failed v0.49 tag unexpectedly exists remotely; v0.50.1 publication refused.");
        var remoteBaseTag = await ReadRemoteTagCommitAsync(root, cancellationToken, ExpectedRemoteBaseTag);
        if (!string.Equals(remoteBaseTag, ExpectedRemoteBase, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted remote v0.49.1 base tag moved; v0.50.1 publication refused.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact accepted v0.49.1 base nor exact v0.50.1 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.50.1 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main disappeared during v0.50.1 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Remote main did not converge to exact accepted v0.50.1 HEAD.");
        if (tagState == "ABSENT")
        {
            await GitAsync(root, cancellationToken, "push", RemoteName, $"refs/tags/{AcceptedTag}:refs/tags/{AcceptedTag}");
            tagPush = true;
        }
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main missing after v0.50.1 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag) ?? throw new InvalidDataException("Remote v0.50.1 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !remoteTagAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.50.1 HEAD.");
        if (await ReadRemoteTagCommitAsync(root, cancellationToken, FailedRemoteTag) is not null) throw new InvalidDataException("Failed v0.50 tag appeared remotely during v0.50.1 publication.");
        if (await ReadRemoteTagCommitAsync(root, cancellationToken, HistoricalFailedRemoteTag) is not null) throw new InvalidDataException("Failed v0.49 tag appeared remotely during v0.50.1 publication.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout)) throw new InvalidDataException("Local Workbench state changed during v0.50.1 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "failed real-host workbench-v0.50-accepted tag is not published",
            "historical failed workbench-v0.49-accepted tag remains absent remotely",
            "the failed v0.50 commit may exist only as an untagged ancestor in the fast-forward chain",
            "no arbitrary remote URL/name",
            "no external Tools/OpenAI/tunnel-client binary published",
            "no runtime credential/tunnel session/local endpoint/readiness failure/private app bytes published",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.50.1 acceptance and successful real-host readiness + ChatGPT read round-trip",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.49.1-accepted",
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.50.1-accepted",
                "git push github-workbench <exact-v0.50.1-head>:refs/heads/main when remote main == exact accepted v0.49.1 base",
                "git push github-workbench refs/tags/workbench-v0.50.1-accepted:refs/tags/workbench-v0.50.1-accepted when target tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit publication of accepted Workbench v0.50.1 stabilization. Failed v0.50 remains only untagged historical ancestry remotely and is not reclassified as an accepted frontier.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.50.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v0501-fixed-tag", AcceptedTag == "workbench-v0.50.1-accepted", AcceptedTag, "workbench-v0.50.1-accepted"),
        ("publisher-v0501-local-parent", ExpectedParent == "bd7b0c4cb69deaee2f575961b9f277e7168485c3", ExpectedParent, "operator-bound local failed-v0.50"),
        ("publisher-v0501-remote-base", ExpectedRemoteBase == "1e0453ccf047bd948e76b577c2395a7d0009ff7a", ExpectedRemoteBase, "accepted remote v0.49.1"),
        ("publisher-v0501-failed-v050-tag-suppressed", FailedRemoteTag == "workbench-v0.50-accepted", FailedRemoteTag, "must remain absent remotely"),
        ("publisher-v0501-conflict-refused", ClassifyRemoteMain("b", "c") == "CONFLICT" && ClassifyRemoteTag("b", "c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT")
    };
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) || !head.Equals(c.Head, StringComparison.OrdinalIgnoreCase) || !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) || !tag.Equals(c.Head, StringComparison.OrdinalIgnoreCase) || !parentTag.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.50.1 frontier changed after publication preview.");
    ...
    private static async Task RequireAncestorAsync(string root, string ancestor, string descendant, CancellationToken ct)
    {
        ...
        if (result.ExitCode != 0) throw new InvalidDataException("Accepted remote v0.49.1 base is not an ancestor of exact local v0.50.1 HEAD.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.50.1 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.50.1 publication Git operation timed out.");
        }
        var o = await stdout;
        var e = await stderr;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.50.1 publication Git operation failed: {e.Trim()}");
