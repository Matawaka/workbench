...
/// <summary>
/// v0.51.8 local checkpoint. Exact predecessor identity is dual-bound by the
/// update transaction build-source manifest and workbench-v0.51.7-accepted at HEAD.
/// MCP owner/index/lease/runtime/evidence files remain excluded from checkpoint authority.
/// </summary>
public sealed class LocalCheckpointV0518Service
{
    public const string Version = "0.51.8";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.51.8";
    public const string ExpectedPredecessorTag = "workbench-v0.51.7-accepted";
    public const string TargetTag = "workbench-v0.51.8-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.51.8 MCP ownership status and stale metadata recovery";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.51.7-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.51.8 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.51.8 local acceptance.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.51.8 tag/version contract.");

        var root = candidate.RepositoryRoot;
        var head = RequireSha((await GitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.51.8 checkpoint preview.");

        var tagHead = RequireSha(
            (await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout,
            ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted local v0.51.7 predecessor tag moved after preview.");

        var status = ParseStatusPaths((await GitAsync(
            root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.51.8 checkpoint preview.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            ...
            ?? throw new InvalidDataException("v0.51.8 validation artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            if (!newParent.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.51.8 checkpoint parent is not exact manifest/tag-bound local v0.51.7 predecessor.");

            await GitAsync(root, cancellationToken,
                "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.51.8: MCP ownership status and stale metadata recovery");
            tagged = true;

            if (!string.IsNullOrWhiteSpace((await GitAsync(
                    root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.51.8 checkpoint.");

            var nonEffects = new[]
            {
                "exact predecessor commit is supplied by the update/build source manifest and independently matched to workbench-v0.51.7-accepted at current HEAD",
                "no remote publication during local checkpoint",
                "no canonical read-lease state, derived active-index runtime state, cross-process index fence, MCP owner file/handle, stale owner evidence archive, MCP adapter or Secure MCP Tunnel state committed",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.51.8",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.51.8 validation + manifest/tag-bound predecessor required",
                false, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.51.8-message>", "git tag -a workbench-v0.51.8-accepted" },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.51.8",
                ...
                $"Local v0.51.8 checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}. Exact predecessor identity came from the update/build manifest and was independently matched to {ExpectedPredecessorTag}.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.51.8-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v0518-predecessor-source", true, "build-source manifest PredecessorGitSha + fixed predecessor tag at current HEAD", "dual binding"),
        ("checkpoint-v0518-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.51.7-accepted", ExpectedPredecessorTag, "workbench-v0.51.7-accepted"),
        ("checkpoint-v0518-target-tag", TargetTag == "workbench-v0.51.8-accepted", TargetTag, "workbench-v0.51.8-accepted"),
        ("checkpoint-v0518-build-schema", BuildManifestSchema == "matawaka.workbench-build-source-manifest/v0.51", BuildManifestSchema, "v0.51"),
        ("checkpoint-v0518-bootstrap-only", true, "AcceptFromBootstrapAsync only", "no manual visible Accept path"),
        ("checkpoint-v0518-runtime-exclusion", true, ".workbench/artifacts/Tools forbidden", "no owner/index/lease/evidence runtime state committed"),
        ("checkpoint-v0518-publication-deferred", true, "no remote mutation in checkpoint", "deferred")
    };
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.51.8 validation receipt is required.");
    }
    ...
    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.51.8 validation artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.51.8 validation artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Validation artifact does not match the passing in-memory v0.51.8 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, "v0.51.8-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.51.8 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema ||
            ...
            throw new InvalidDataException("Unexpected v0.51.8 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.51.8 build source manifest.");
        ...
        foreach (var item in bound)
        {
            ...
            if (!full.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full) ||
                ...
                throw new InvalidDataException($"Build-bound v0.51.8 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) ||
            ...
            throw new InvalidDataException("Running executable digest does not match v0.51.8 validation receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.51.8 checkpoint Git process.");
        ...
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.51.8 checkpoint Git operation failed: {error.Trim()}");
