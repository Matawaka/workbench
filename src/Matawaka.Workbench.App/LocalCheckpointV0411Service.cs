...
public sealed class LocalCheckpointV0411Service
{
    public const string Version = "0.41.1";
    public const string ExpectedPredecessorCommit = "1ae2ccd8a0d789c46b26ac29970e2a1a697d8763";
    public const string ExpectedPredecessorTag = "workbench-v0.41-accepted";
    public const string TargetTag = "workbench-v0.41.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.41.1 JSON search visible-selection stabilization";
    ...
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != "matawaka.workbench-acceptance-receipt/v0.41.1")
            throw new InvalidDataException("A passing Workbench v0.41.1 Self-test receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.41 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.41 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.41.1 source changes to checkpoint.");
    ...
    public Task<LocalCheckpointReceipt> AcceptAsync(LocalCheckpointCandidate candidate, CancellationToken cancellationToken)
        => AcceptCoreAsync(
            ...
            "explicit v0.41.1 Accept confirmation after passing v0.41.1 Self-test",
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed v0.40 transition-bootstrap lease id is required for automatic v0.41.1 Accept.");
        return AcceptCoreAsync(
            candidate,
            "human-operator-via-one-shot-transition-bootstrap",
            $"one-shot v0.40 transition-bootstrap lease {bootstrapLeaseId}; originally authorized by explicit Update Workbench confirmation; exact v0.41.1 Self-test Passed=true required",
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.41.1 contract.");

        var root = candidate.RepositoryRoot;
        var head = RequireSha((await RunGitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.41.1 checkpoint preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.41.1 checkpoint preview.");
        ...
        var acceptance = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.41.1 acceptance artifact disappeared before checkpoint.");
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != "matawaka.workbench-acceptance-receipt/v0.41.1")
            throw new InvalidDataException("v0.41.1 local Accept requires a still-passing exact acceptance artifact.");
        ...
        try
        {
            ...
            await RunGitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m",
                "Accepted Workbench v0.41.1: JSON search visible-selection stabilization; v0.41 search semantics unchanged");
            tagged = true;

            if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.41.1 checkpoint.");
            ...
            {
                "git add -A -- .",
                "git commit -m <fixed-v0.41.1-message>",
                "git tag -a workbench-v0.41.1-accepted"
            };
            var authority = new LocalCheckpointAuthorityReceipt(
                bootstrapLeaseId is null
                    ? "matawaka.workbench-local-checkpoint-authority-receipt/v0.41.1"
                    : "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.41.1",
                subject,
            ...
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.41.1", Version, DateTimeOffset.Now,
                ...
                bootstrapLeaseId is null
                    ? "Local v0.41.1 checkpoint after explicit manual Accept. Publish/Lifecycle remain separate."
                    : $"Local v0.41.1 checkpoint consumed one-shot v0.40 transition-bootstrap lease {bootstrapLeaseId} only after exact first-boot Self-test Passed=true. This creates no reusable acceptance or publication authority.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.41.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v0411-fixed-predecessor", ExpectedPredecessorCommit == "1ae2ccd8a0d789c46b26ac29970e2a1a697d8763", ExpectedPredecessorCommit, "accepted v0.41"),
        ("checkpoint-v0411-fixed-target-tag", TargetTag == "workbench-v0.41.1-accepted", TargetTag, "workbench-v0.41.1-accepted"),
        ("checkpoint-v0411-bootstrap-is-not-ui-confirmation", true, "bootstrap authority uses ExplicitUiConfirmationRequired=false", "false"),
        ("checkpoint-v0411-publish-separate", true, "no push/fetch in allowed Git operations", "separate")
    };

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.41.1 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.41.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != "matawaka.workbench-acceptance-receipt/v0.41.1" || parsed.Version != Version ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.41.1 Self-test receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, "v0.41.1-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.41.1 build source manifest is missing.");
        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.41.1 build source manifest could not be parsed.");
        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.41" || manifest.Version != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.41.1 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.41.1 build source manifest.");
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.41.1 Self-test receipt.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.41.1 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.41.1 checkpoint Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.41.1 checkpoint Git operation failed: {stderr.Trim()}");
