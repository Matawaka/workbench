...
public sealed class LocalCheckpointV0511Service
{
    public const string Version = "0.51.1";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.51.1";
    public const string ExpectedPredecessorCommit = "a8a93143e942a02913475013e355d61b2fa6bee8";
    public const string ExpectedPredecessorTag = "workbench-v0.51-accepted";
    public const string TargetTag = "workbench-v0.51.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.51.1 read lease to local MCP auto-start";
        ...
        CancellationToken cancellationToken)
    {
        ...
        var head = RequireSha(
            (await GitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout,
            "HEAD");
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                $"HEAD {head} is not exact accepted local v0.51 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!predecessorTagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "workbench-v0.51-accepted is not at exact current HEAD.");

        if (!string.IsNullOrWhiteSpace(
                (await GitAsync(root, cancellationToken, "tag", "--list", TargetTag)).Stdout))
            throw new InvalidDataException($"Target tag already exists: {TargetTag}");

        var userName = (await GitAsync(
            root, cancellationToken, true, "config", "--get", "user.name")).Stdout.Trim();
        var userEmail = (await GitAsync(
            root, cancellationToken, true, "config", "--get", "user.email")).Stdout.Trim();
        if (string.IsNullOrWhiteSpace(userName) || string.IsNullOrWhiteSpace(userEmail))
            throw new InvalidDataException("Local Git identity is missing.");

        var changed = ParseStatusPaths(
            (await GitAsync(
                root, cancellationToken,
                "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (changed.Count == 0)
            throw new InvalidDataException(
                "There are no Workbench v0.51.1 source changes to checkpoint.");
        if (changed.Any(IsForbiddenCheckpointPath))
            throw new InvalidDataException(
                "Checkpoint contains forbidden artifacts/.workbench/Tools runtime path.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException(
                "A claimed transition-bootstrap lease id is required for automatic v0.51.1 local acceptance.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            candidate.ExpectedPredecessorTag != ExpectedPredecessorTag ||
            candidate.TargetTag != TargetTag)
            throw new InvalidDataException(
                "Checkpoint candidate does not match fixed v0.51.1 contract.");
        ...
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "Workbench HEAD changed after v0.51.1 checkpoint preview.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "Accepted local v0.51 predecessor tag moved after preview.");

        var status = ParseStatusPaths(
            (await GitAsync(
                root, cancellationToken,
                "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException(
                "Workbench working tree changed after v0.51.1 checkpoint preview.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            ...
            ?? throw new InvalidDataException(
                "v0.51.1 validation artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            var newHead = RequireSha(
                (await GitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout,
                "new HEAD");
            var newParent = RequireSha(
                (await GitAsync(root, cancellationToken, "rev-parse", "HEAD^")).Stdout,
                "new HEAD parent");
            if (!newParent.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException(
                    "v0.51.1 checkpoint parent is not exact accepted local v0.51 predecessor.");

            await GitAsync(
                root, cancellationToken,
                "tag", "-a", TargetTag,
                "-m", "Accepted Workbench v0.51.1: read lease to local MCP auto-start");
            tagged = true;
            ...
            if (!string.IsNullOrWhiteSpace(
                    (await GitAsync(
                        ...
                        "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException(
                    "Workbench working tree is not clean after v0.51.1 checkpoint.");

            var nonEffects = new[]
            {
                "no remote publication during local checkpoint",
                "no read lease, MCP adapter or Secure MCP Tunnel runtime state committed",
                "no bearer plaintext, local MCP endpoint or clipboard contents committed",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.51.1",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.51.1 validation Passed=true required",
                false,
                true,
                false,
                false,
                ...
                {
                    "git add -A -- .",
                    "git commit -m <fixed-v0.51.1-message>",
                    "git tag -a workbench-v0.51.1-accepted"
                },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.51.1",
                Version,
                DateTimeOffset.Now,
                ...
                true,
                nonEffects,
                $"Local v0.51.1 checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}. Runtime lease/MCP/clipboard/tunnel/private app state remains outside checkpoint payload.");
        }
        catch
        {
            if (tagged)
                await GitAsync(
                    root, CancellationToken.None, true,
                    "tag", "-d", TargetTag);
            if (committed)
                await GitAsync(
                    root, CancellationToken.None, true,
                    "reset", "--mixed", candidate.PreviousHead);
            else
                await GitAsync(
                    root, CancellationToken.None, true, "reset");
            throw;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        var dir = Path.Combine(
            ResolveRepositoryRoot(workspaceRoot), "artifacts", "acceptance");
        Directory.CreateDirectory(dir);
        var path = Path.Combine(
            dir, $"checkpoint-v0.51.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    public static IReadOnlyList<(string Id, bool Passed, string Observed, string Expected)>
        RunOfflineContractChecks()
        => new[]
        {
            ("checkpoint-v0511-fixed-predecessor",
                ExpectedPredecessorCommit == "a8a93143e942a02913475013e355d61b2fa6bee8",
                ExpectedPredecessorCommit,
                "accepted local v0.51"),
            ("checkpoint-v0511-fixed-predecessor-tag",
                ExpectedPredecessorTag == "workbench-v0.51-accepted",
                ExpectedPredecessorTag,
                "workbench-v0.51-accepted"),
            ("checkpoint-v0511-fixed-target-tag",
                TargetTag == "workbench-v0.51.1-accepted",
                TargetTag,
                "workbench-v0.51.1-accepted"),
            ("checkpoint-v0511-bootstrap-only",
                true,
                "AcceptFromBootstrapAsync only",
                "no manual visible Accept path"),
            ("checkpoint-v0511-publication-deferred",
                true,
                "Publish handler is fail-closed/no remote mutation",
                "public v0.51 gate unresolved"),
            ("checkpoint-v0511-runtime-secrets-not-git",
                true,
                "artifacts/.workbench/Tools forbidden checkpoint path",
                "not checkpoint payload")
        };
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed ||
            ...
            throw new InvalidDataException(
                "A passing exact Workbench v0.51.1 validation receipt is required.");
    }
        ...
        WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException(
                "Passing v0.51.1 validation artifact is missing.");

        var full = Path.GetFullPath(artifactPath);
        var allowed = Path.GetFullPath(
            Path.Combine(root, "artifacts", "acceptance")) + Path.DirectorySeparatorChar;
        if (!full.StartsWith(allowed, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "Acceptance artifact must be under Workbench/artifacts/acceptance.");

        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException(
                "v0.51.1 validation artifact could not be parsed.");

        if (!parsed.Passed ||
            parsed.Schema != AcceptanceSchema ||
            parsed.Version != Version ||
            parsed.RunId != acceptance.RunId ||
            !parsed.AppExecutableSha256.Equals(
                acceptance.AppExecutableSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "Validation artifact does not match the passing in-memory v0.51.1 receipt.");
        ...
        IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(
                    dir, "v0.51.1-source-manifest*.json")
        ...
        if (path is null)
            throw new InvalidDataException(
                "v0.51.1 build source manifest is missing.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(
            File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException(
                "v0.51.1 build source manifest could not be parsed.");

        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.51" ||
            manifest.Version != Version ||
            !manifest.PredecessorGitSha.Equals(
                predecessor, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "Unexpected v0.51.1 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException(
                "Changed-file set differs from v0.51.1 build source manifest.");
        ...
        foreach (var item in bound)
        {
            ...
            if (!full.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) ||
                ...
                throw new InvalidDataException(
                    $"Build-bound v0.51.1 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) ||
            ...
            throw new InvalidDataException(
                "Running executable digest does not match v0.51.1 validation receipt.");
    }
    ...
    private static string ResolveRepositoryRoot(string workspaceRoot)
    {
        var root = Path.GetFullPath(
            Path.Combine(workspaceRoot.Trim(), "Workbench"));
        if (!Directory.Exists(Path.Combine(root, ".git")))
            throw new InvalidDataException(
                $"Workbench Git repository missing: {root}");
        return root;
    }
    ...
    private static string RequireSha(string value, string role)
    {
        ...
        if (sha.Length != 40 || sha.Any(ch => !Uri.IsHexDigit(ch)))
            throw new InvalidDataException(
                $"{role} is not a Git SHA-1: {sha}");
        return sha.ToLowerInvariant();
    }

    private static Task<GitResult> GitAsync(
        string root,
        CancellationToken cancellationToken,
        params string[] args)
        ...
        params string[] args)
    {
        ...
        if (!process.Start())
            throw new InvalidDataException(
                "Failed to start fixed v0.51.1 checkpoint Git process.");
        ...
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException(
                $"v0.51.1 checkpoint Git operation failed: {error.Trim()}");
