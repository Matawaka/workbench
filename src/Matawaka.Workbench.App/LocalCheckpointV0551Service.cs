...
public sealed record LocalCheckpointCandidateV0551(
    string Version,
...
public sealed record LocalCheckpointReceiptV0551(
    string Schema,
    string Version,
...
public sealed class LocalCheckpointV0551Service
{
    public const string Version = "0.55.1";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.55.1";
    public const string ExpectedPredecessorTag = "workbench-v0.55-accepted";
    public const string ExpectedPredecessorCommit = "02d81b8559bc7c9676949be0557d20ecb50a9890";
    public const string TargetTag = "workbench-v0.55.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.55.1 real-host invocation admission and fixed publication closure";
    public const string BuildManifestSchema = "matawaka.workbench-build-source-manifest/v0.55";
    public const string BuildManifestPattern = "v0.55.1-source-manifest*.json";

    private static readonly TimeSpan GitTimeout = TimeSpan.FromSeconds(30);
    private static readonly JsonSerializerOptions JsonOptions = new() { PropertyNameCaseInsensitive = true, WriteIndented = true };

    public async Task<LocalCheckpointCandidateV0551> PreviewAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.55.1 exact local predecessor mismatch: expected {ExpectedPredecessorCommit}, observed {head}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.55-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.55.1 source changes to checkpoint.");
        if (changed.Any(IsForbiddenCheckpointPath))
            throw new InvalidDataException("v0.55.1 checkpoint contains forbidden runtime/CI/test/package paths.");
        var manifest = ValidateBuildSourceManifest(root, head, changed);

        return new LocalCheckpointCandidateV0551(
    ...
    public async Task<LocalCheckpointReceiptV0551> AcceptFromBootstrapAsync(
        LocalCheckpointCandidateV0551 candidate,
        string bootstrapLeaseId,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.55.1 local acceptance.");
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.55.1 contract.");
        ...
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase) || !head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.55.1 checkpoint preview.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.55.1 checkpoint preview.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.55.1 acceptance artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            if (!newParent.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.55.1 checkpoint parent is not exact local accepted v0.55 predecessor.");
            await GitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.55.1: real-host invocation admission and fixed publication closure");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.55.1 checkpoint.");

            return new LocalCheckpointReceiptV0551(
                "matawaka.workbench-local-checkpoint-receipt/v0.55.1", Version, DateTimeOffset.Now,
                ...
                $"Local v0.55.1 publication-closure checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}; exact predecessor was {ExpectedPredecessorCommit} / {ExpectedPredecessorTag}. Publication remains separate.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(
        string workspaceRoot,
        LocalCheckpointReceiptV0551 receipt,
        CancellationToken cancellationToken)
    {
        var dir = Path.Combine(ResolveRepositoryRoot(workspaceRoot), "artifacts", "acceptance");
        Directory.CreateDirectory(dir);
        var path = Path.Combine(dir, $"checkpoint-v0.55.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v0551-version", Version == "0.55.1", Version, "0.55.1"),
        ("checkpoint-v0551-predecessor", ExpectedPredecessorCommit == "02d81b8559bc7c9676949be0557d20ecb50a9890", ExpectedPredecessorCommit, "exact local accepted v0.55"),
        ("checkpoint-v0551-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.55-accepted", ExpectedPredecessorTag, "workbench-v0.55-accepted"),
        ("checkpoint-v0551-target-tag", TargetTag == "workbench-v0.55.1-accepted", TargetTag, "workbench-v0.55.1-accepted"),
        ("checkpoint-v0551-build-manifest", BuildManifestSchema == "matawaka.workbench-build-source-manifest/v0.55" && BuildManifestPattern == "v0.55.1-source-manifest*.json", BuildManifestSchema, "v0.55 major.minor schema + exact target filename"),
        ("checkpoint-v0551-publication", true, "RemotePushAllowed=false during checkpoint", "separate explicit Publish accepted confirmation")
    };

    private static IReadOnlyList<string> NonEffects() => new[]
    {
        "exact local predecessor is workbench-v0.55-accepted at 02d81b8559bc7c9676949be0557d20ecb50a9890",
        "local checkpoint performs no remote publication or network access",
        "real-host model-invocation evidence is read-only admission evidence",
        "v0.52 acquisition, v0.53 execution, v0.54 materialization and v0.55 invocation primitives are not widened",
        "no artifact acquisition/materialization/process/model invocation/benchmark/game/display authority",
        "no catalog mutation or Agent Execute/ActionPermit/SuccessorPermit"
    };
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.55.1 acceptance receipt is required.");
    }
    ...
    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.55.1 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.55.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match passing in-memory v0.55.1 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        if (path is null) throw new InvalidDataException("v0.55.1 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema || obj.GetProperty("Version").GetString() != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.55.1 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.55.1 build source manifest.");
        foreach (var item in bound)
        {
            ...
            if (!File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Build-bound v0.55.1 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.55.1 acceptance receipt.");
    }
