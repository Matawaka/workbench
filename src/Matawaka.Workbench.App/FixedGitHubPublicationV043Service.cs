...
public sealed class FixedGitHubPublicationV043Service
{
    ...
    public const string AcceptedTag = "workbench-v0.43-accepted";
    public const string ExpectedParent = "2a68f43b7c8b3a5e8aae7d960f2d997ffa7ec525";
    public const string ExpectedParentTag = "workbench-v0.42-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.43";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.43";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.43 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.43 accepted HEAD parent is not exact accepted v0.42: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.42 predecessor tag does not point at exact v0.43 parent.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.43 tag does not point at current HEAD.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.43 contract.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact v0.42 parent nor exact v0.43 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.43 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main disappeared during v0.43 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main did not converge to exact accepted v0.43 HEAD.");
        if (tagState == "ABSENT")
        {
        ...
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main missing after v0.43 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag)
            ?? throw new InvalidDataException("Remote v0.43 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.43 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.43 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "clickable app chips/tree tabs create no publication or application authority",
            "tree observation reads no application file contents",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.43 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.43-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact v0.42 parent",
                "git push github-workbench refs/tags/workbench-v0.43-accepted:refs/tags/workbench-v0.43-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted v0.43 clickable read-only application tree tabs. Structural observation does not extend application or publication authority.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.43-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v043-fixed-tag", AcceptedTag == "workbench-v0.43-accepted", AcceptedTag, "workbench-v0.43-accepted"),
        ("publisher-v043-fixed-parent", ExpectedParent == "2a68f43b7c8b3a5e8aae7d960f2d997ffa7ec525", ExpectedParent, "accepted v0.42"),
        ("publisher-v043-parent-tag", ExpectedParentTag == "workbench-v0.42-accepted", ExpectedParentTag, "workbench-v0.42-accepted"),
        ("publisher-v043-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v043-tree-observation-not-authority", true, "explicit Publish button only", "separate from app tree observation")
    };
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            ...
            throw new InvalidDataException("Local accepted v0.43 frontier changed after publication preview.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.43 publication Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.43 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.43 publication Git operation failed: {stderr.Trim()}");
