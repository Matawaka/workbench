...
/// <summary>
/// v0.33 local acceptance successor over exact accepted v0.32.
/// It consumes only a passing v0.33 Self-test receipt and a byte-bound v0.33
/// build-source manifest. Remote publication remains a separate later decision.
/// </summary>
public sealed class LocalCheckpointV033Service
{
    public const string Version = "0.33.0";
    public const string ExpectedPredecessorTag = "workbench-v0.32-accepted";
    public const string TargetTag = "workbench-v0.33-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.33 accepted maintenance update orchestrator";
        ...
        CancellationToken cancellationToken)
    {
        if (!acceptance.Passed ||
            ...
            !string.Equals(acceptance.Schema, "matawaka.workbench-acceptance-receipt/v0.33", StringComparison.Ordinal))
            throw new InvalidDataException("A passing Workbench v0.33 Self-test receipt is required before local checkpoint acceptance.");
        ...
        if (changedFiles.Count == 0)
            throw new InvalidDataException("There are no Workbench v0.33 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match v0.33 fixed acceptance contract.");
        ...
        if (!string.Equals(headBefore, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.33 checkpoint preview.");
        ...
        if (!currentStatus.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.33 checkpoint preview.");
        ...
        try
        {
            ...
            if (string.Equals(newHead, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Git commit did not advance v0.33 HEAD.");

            await RunGitAsync(repositoryRoot, cancellationToken,
                "tag", "-a", TargetTag,
                "-m", "Accepted Workbench v0.33: one Update candidate operator session sequences the existing typed update gates, preserves fresh sub-receipts and fail-closed rollback, and still requires separate candidate launch, Self-test, checkpoint and publication decisions");
            tagCreated = true;
            ...
            if (!string.IsNullOrWhiteSpace(cleanStatus))
                throw new InvalidDataException("Workbench working tree is not clean after v0.33 checkpoint commit.");
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.33",
                "human-operator-at-workbench-ui",
                "git.local-checkpoint.accept",
                repositoryRoot,
                "explicit v0.33 Accept checkpoint button + confirmation dialog after passing v0.33 Self-test",
                true,
                true,
                false,
                false,
                false,
                false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.33-message>", "git tag -a workbench-v0.33-accepted" },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.33",
                Version,
                DateTimeOffset.Now,
                ...
                true,
                nonEffects,
                "Local Workbench v0.33 acceptance only. Remote publication remains a separate later Publish accepted decision and is not implied by this checkpoint.");
        }
        catch
        {
            if (tagCreated)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.33-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
        ...
        WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.33 acceptance artifact file is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.33 acceptance artifact could not be parsed.");
        if (!parsed.Passed || !string.Equals(parsed.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.33 Self-test receipt.");
        return full;
    }
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(checkpointDir, "v0.33.0-source-manifest*.json", SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.33 build source manifest is missing. Refusing to checkpoint source that is not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.33 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.33", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Unexpected v0.33 build source manifest identity/predecessor.");
        ...
        if (!manifestFiles.Select(item => item.Path).SequenceEqual(currentFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Current Workbench changed-file set differs from the v0.33 source set captured at build time.");
        ...
        foreach (var item in manifestFiles)
        {
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full))
                throw new InvalidDataException($"Build-bound v0.33 source file missing or escapes Workbench root: {item.Path}");
            if (!string.Equals(HashFile(full), item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"v0.33 source file changed after candidate build: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expectedSha256)
    {
        ...
        if (!string.Equals(HashFile(path), expectedSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest no longer matches v0.33 Self-test receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.33 checkpoint git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.33 checkpoint git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.33 checkpoint git operation failed: {stderr.Trim()}");
