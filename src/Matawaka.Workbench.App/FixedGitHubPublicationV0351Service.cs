...
/// <summary>
/// Fixed GitHub publication successor for the v0.35.1 stabilization patch.
/// Preserves the accepted one-repository, fast-forward-only, non-force boundary.
/// </summary>
public sealed class FixedGitHubPublicationV0351Service
{
    ...
    public const string AcceptedTag = "workbench-v0.35.1-accepted";
    public const string ExpectedParent = "689cdf5ef2f9f403efe09bb251c91da1c5951ec6";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.35.1";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.35.1";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.35.1 accepted-source publication.");
        ...
        if (!string.Equals(parent, ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.35.1 accepted HEAD parent is not exact accepted v0.35: {parent}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.35.1 stabilization publication contract.");
        ...
        if (!string.Equals(remoteMainAfter, candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Fixed remote main/tag readback does not equal exact accepted local v0.35.1 HEAD.");
        ...
        if (!string.Equals(headAfter, candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench HEAD or working tree changed during v0.35.1 publication.");
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted button + separate confirmation dialog after local v0.35.1 stabilization acceptance",
            true,
            true,
            true,
            false,
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.35.1-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact parent",
                "git push github-workbench refs/tags/workbench-v0.35.1-accepted:refs/tags/workbench-v0.35.1-accepted when remote tag absent"
            },
            nonEffects);
        ...
        return new FixedGitHubPublicationReceipt(
            ...
            authority,
            nonEffects,
            "Explicit human maintenance publication of one already-accepted Workbench v0.35.1 stabilization checkpoint. This does not create lifecycle/local-app/AgentExecute/general-network authority.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"fixed-github-publication-v0.35.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    public static IReadOnlyList<(string Id, bool Passed, string Observed, string Expected)> RunOfflineContractChecks()
        => new[]
        {
            ("publisher-v0351-fixed-remote-name", RemoteName == "github-workbench", RemoteName, "github-workbench"),
            ("publisher-v0351-fixed-remote-url", RemoteUrl == "https://github.com/Matawaka/workbench.git", RemoteUrl, "fixed Matawaka/workbench URL"),
            ("publisher-v0351-fixed-tag", AcceptedTag == "workbench-v0.35.1-accepted", AcceptedTag, "workbench-v0.35.1-accepted"),
            ("publisher-v0351-fixed-parent", ExpectedParent == "689cdf5ef2f9f403efe09bb251c91da1c5951ec6", ExpectedParent, "accepted v0.35"),
            ("publisher-v0351-parent-fast-forward", ClassifyRemoteMain("a", "b", "a") == "PARENT", ClassifyRemoteMain("a", "b", "a"), "PARENT"),
            ("publisher-v0351-idempotent-main", ClassifyRemoteMain("a", "b", "b") == "ALREADY_HEAD", ClassifyRemoteMain("a", "b", "b"), "ALREADY_HEAD"),
            ("publisher-v0351-conflicting-main-refused", ClassifyRemoteMain("a", "b", "c") == "CONFLICT", ClassifyRemoteMain("a", "b", "c"), "CONFLICT"),
            ("publisher-v0351-absent-tag-admitted", ClassifyRemoteTag("b", null) == "ABSENT", ClassifyRemoteTag("b", null), "ABSENT"),
            ("publisher-v0351-idempotent-tag", ClassifyRemoteTag("b", "b") == "ALREADY_HEAD", ClassifyRemoteTag("b", "b"), "ALREADY_HEAD"),
            ("publisher-v0351-conflicting-tag-refused", ClassifyRemoteTag("b", "c") == "CONFLICT", ClassifyRemoteTag("b", "c"), "CONFLICT")
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout))
            throw new InvalidDataException("Workbench working tree changed after v0.35.1 publication preview.");
        ...
        if (!string.Equals(head, candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Local accepted Workbench v0.35.1 frontier changed after publication preview.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start())
            throw new InvalidDataException("Failed to start fixed v0.35.1 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.35.1 publication Git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.35.1 publication Git operation failed ({string.Join(' ', args)}): {stderr.Trim()}");
