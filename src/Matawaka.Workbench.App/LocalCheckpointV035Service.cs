...
/// <summary>
/// v0.35 local acceptance successor over exact accepted v0.34.1.
/// It accepts only byte-bound Workbench source after a passing v0.35 Self-test.
/// Remote publication, lifecycle assessment and local-app updates remain separate authorities.
/// </summary>
public sealed class LocalCheckpointV035Service
{
    public const string Version = "0.35.0";
    public const string ExpectedPredecessorCommit = "c69d3237bae06b80481ce2421eb34e8cf1a88c1b";
    public const string ExpectedPredecessorTag = "workbench-v0.34.1-accepted";
    public const string TargetTag = "workbench-v0.35-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.35 product surface and local application maintenance";
        ...
        CancellationToken cancellationToken)
    {
        if (!acceptance.Passed ||
            ...
            !string.Equals(acceptance.Schema, "matawaka.workbench-acceptance-receipt/v0.35", StringComparison.Ordinal))
            throw new InvalidDataException("A passing Workbench v0.35 Self-test receipt is required before local checkpoint acceptance.");
        ...
        if (!string.Equals(currentHead, ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {currentHead} is not exact accepted v0.34.1 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (changedFiles.Count == 0)
            throw new InvalidDataException("There are no Workbench v0.35 source changes to checkpoint.");
        if (changedFiles.Any(IsForbiddenCheckpointPath))
            throw new InvalidDataException("v0.35 checkpoint contains a forbidden artifacts/.workbench path.");
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match the fixed v0.35 acceptance contract.");
        ...
        if (!string.Equals(headBefore, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.35 checkpoint preview.");

        var currentStatus = ParseStatusPaths(
            (await RunGitAsync(repositoryRoot, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (!currentStatus.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.35 checkpoint preview.");
        ...
        try
        {
            ...
            if (string.Equals(newHead, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Git commit did not advance v0.35 HEAD.");
            ...
            await RunGitAsync(
                ...
                "tag", "-a", TargetTag,
                "-m",
                "Accepted Workbench v0.35: reduced active product surface plus bounded local application maintenance under fixed workspace Apps roots; no general network/process/filesystem authority");
            tagCreated = true;
            ...
            if (!string.IsNullOrWhiteSpace(cleanStatus))
                throw new InvalidDataException("Workbench working tree is not clean after v0.35 checkpoint commit.");
            ...
            var nonEffects = new[]
            {
                ...
                "no lifecycle authority inferred from checkpoint",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.35",
                "human-operator-at-workbench-ui",
                "git.local-checkpoint.accept",
                repositoryRoot,
                "explicit v0.35 Accept checkpoint button + confirmation after passing v0.35 Self-test",
                true,
                true,
                false,
                false,
                ...
                {
                    "git add -A -- .",
                    "git commit -m <fixed-v0.35-message>",
                    "git tag -a workbench-v0.35-accepted"
                },
                nonEffects);
            ...
            return new LocalCheckpointReceipt(
                ...
                true,
                nonEffects,
                "Local Workbench v0.35 acceptance only. Remote publication and successor-generic lifecycle assessment remain two separate later decisions; local-app update authority is unrelated and is not inferred from Workbench acceptance.");
        }
        catch
        {
            if (tagCreated)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.35-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
        ...
        WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.35 acceptance artifact file is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            ...
            ?? throw new InvalidDataException("v0.35 acceptance artifact could not be parsed.");
        if (!parsed.Passed ||
            !string.Equals(parsed.Schema, "matawaka.workbench-acceptance-receipt/v0.35", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.35 Self-test receipt.");
        return full;
    }
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(
                    checkpointDir,
                    "v0.35.0-source-manifest*.json",
                    SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.35 build source manifest is missing. Refusing source not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(
            File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.35 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.35", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Unexpected v0.35 build source manifest identity/predecessor.");
        ...
        if (!manifestFiles.Select(item => item.Path).SequenceEqual(currentFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Current changed-file set differs from the v0.35 source set captured at build time.");
        ...
        foreach (var item in manifestFiles)
        {
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full))
                throw new InvalidDataException($"Build-bound v0.35 source file missing or escapes Workbench root: {item.Path}");
            if (!string.Equals(HashFile(full), item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"v0.35 source file changed after candidate build: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expectedSha256)
    {
        ...
        if (!string.Equals(HashFile(path), expectedSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest no longer matches v0.35 Self-test receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start())
            throw new InvalidDataException("Failed to start fixed v0.35 checkpoint Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.35 checkpoint Git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.35 checkpoint Git operation failed: {stderr.Trim()}");
