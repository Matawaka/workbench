...
public sealed class LocalCheckpointService
{
    ...
    public const string ExpectedPredecessorTag = "workbench-v0.26-accepted";
    public const string TargetTag = "workbench-v0.27-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.27 accepted transport adversarial independence controls";
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await RunGitAsync(repositoryRoot, cancellationToken,
                ...
                "-m", "Accepted Workbench v0.27: adversarial transport-copy byte drift, extra ZIP entries, and manifest drift are refused before evidence materialization, while preserving producer-authentication, cross-machine portability, live recovery, and Stable Core non-claims");
            tagCreated = true;
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.27",
                ...
                false,
                false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.27-message>", "git tag -a <fixed-v0.27-tag>" },
                new[]
                {
            ...
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.27",
                Version,
                DateTimeOffset.Now,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.27-{DateTime.Now:yyyyMMdd-HHmmss}.json");
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(checkpointDir, "v0.27.0-source-manifest*.json", SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.27 build source manifest is missing. Refusing to checkpoint source that is not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.27 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.27", StringComparison.Ordinal) ||
            !string.Equals(manifest.Version, Version, StringComparison.Ordinal))
            throw new InvalidDataException("Unexpected v0.27 build source manifest schema/version.");
