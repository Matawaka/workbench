...
public sealed class LocalCheckpointService
{
    ...
    public const string ExpectedPredecessorTag = "workbench-v0.19-accepted";
    public const string TargetTag = "workbench-v0.20-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.20 accepted recovery capability admission gate";
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await RunGitAsync(repositoryRoot, cancellationToken,
                ...
                "-m", "Accepted Workbench v0.20: recovery capability admission remains evidence-bounded, read-only, and separate from recovery execution or Stable Core promotion authority");
            tagCreated = true;
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.20",
                ...
                false,
                false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.20-message>", "git tag -a <fixed-v0.20-tag>" },
                new[]
                {
            ...
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.20",
                Version,
                DateTimeOffset.Now,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.20-{DateTime.Now:yyyyMMdd-HHmmss}.json");
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(checkpointDir, "v0.20.0-source-manifest*.json", SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.20 build source manifest is missing. Refusing to checkpoint source that is not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.20 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.20", StringComparison.Ordinal) ||
            !string.Equals(manifest.Version, Version, StringComparison.Ordinal))
            throw new InvalidDataException("Unexpected v0.20 build source manifest schema/version.");
