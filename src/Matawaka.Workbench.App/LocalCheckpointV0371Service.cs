...
public sealed class LocalCheckpointV0371Service
{
    public const string Version = "0.37.1";
    public const string ExpectedPredecessorCommit = "0d20e3bbe7c28b48cac3ef97b903b4a3a6176521";
    public const string ExpectedPredecessorTag = "workbench-v0.37-accepted";
    public const string TargetTag = "workbench-v0.37.1-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.37.1 candidate managed-root role separation";
    ...
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != "matawaka.workbench-acceptance-receipt/v0.37.1")
            throw new InvalidDataException("A passing Workbench v0.37.1 Self-test receipt is required.");
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {head} is not exact accepted v0.37 predecessor {ExpectedPredecessorCommit}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.37 predecessor tag is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.37.1 source changes to checkpoint.");
    ...
    public async Task<LocalCheckpointReceipt> AcceptAsync(LocalCheckpointCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.37.1 contract.");
        ...
        var committed = false; var tagged = false;
        try
        {
            ...
            await RunGitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m",
                "Accepted Workbench v0.37.1: Candidate Source != Managed Application role separation; no candidate import/update/launch authority");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.37.1 checkpoint.");
            var nonEffects = new[]
            {
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.37.1",
                "human-operator-at-workbench-ui", "git.local-checkpoint.accept", root,
                "explicit v0.37.1 Accept confirmation after passing v0.37.1 Self-test",
                true, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.37.1-message>", "git tag -a workbench-v0.37.1-accepted" },
                nonEffects);
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.37.1", Version, DateTimeOffset.Now,
                ...
                "Local v0.37.1 stabilization checkpoint only. Candidate import/update/publication/lifecycle remain separate.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.37.1-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.37.1 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.37.1 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != "matawaka.workbench-acceptance-receipt/v0.37.1" || parsed.Version != Version ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.37.1 Self-test receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir) ? Directory.GetFiles(dir, "v0.37.1-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault() : null;
        if (path is null) throw new InvalidDataException("v0.37.1 build source manifest is missing.");
        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(path, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.37.1 build source manifest could not be parsed.");
        if (manifest.Schema != "matawaka.workbench-build-source-manifest/v0.37" || manifest.Version != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.37.1 build source manifest identity/predecessor.");
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.37.1 Self-test receipt.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        using var process = new Process { StartInfo = psi }; if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.37.1 checkpoint Git process.");
        ...
        try { await process.WaitForExitAsync(timeout.Token); } catch (OperationCanceledException) when (!ct.IsCancellationRequested) { try { process.Kill(true); } catch { } throw new InvalidDataException("v0.37.1 checkpoint Git operation timed out."); }
        var stdout = await stdoutTask; var stderr = await stderrTask; if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.37.1 checkpoint Git operation failed: {stderr.Trim()}");
