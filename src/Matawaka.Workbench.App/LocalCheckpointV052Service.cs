...
public sealed class LocalCheckpointV052Service
{
    public const string Version = "0.52";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.52";
    public const string ExpectedPredecessorTag = "workbench-v0.51.13-accepted";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.52 exact local predecessor mismatch: expected {ExpectedPredecessorCommit}, observed {head}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.51.13-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.52 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.52 local acceptance.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.52 contract.");
        ...
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Workbench HEAD changed after v0.52 checkpoint preview.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted local v0.51.13 predecessor tag moved after preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.52 checkpoint preview.");
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(
            ...
            ?? throw new InvalidDataException("v0.52 acceptance artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            if (!newParent.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.52 checkpoint parent is not the exact local v0.51.13 accepted predecessor.");

            await GitAsync(root, cancellationToken,
                "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.52: bounded artifact acquisition");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.52 checkpoint.");
            ...
            var nonEffects = new[]
            {
                ...
                "no artifact acquisition lease/transaction/partial/final downloaded bytes committed",
                "no bearer plaintext/hash or transient acquisition authority committed",
                "no local-app read lease/index/fence/MCP owner/generation/binding/listener/shutdown runtime state committed",
                "generic v0.52 acquisition source does not grant extraction/install/execute/runtime/benchmark/model/game authority",
                "KONTUR intake/anchors are declarative integration contracts only, not download or runtime authority",
                "no private Apps/AppSources bytes committed",
                "no automatic retry/resume/Publish/Lifecycle authority",
                "no catalog mutation or Agent Execute/ActionPermit"
            };

            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.52",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.52 validation + local predecessor binding required",
                false, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.52-message>", "git tag -a workbench-v0.52-accepted" },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.52",
                ...
                $"Local v0.52 bounded-artifact-acquisition checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}; exact predecessor was {ExpectedPredecessorCommit} / {ExpectedPredecessorTag}.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.52-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.52 acceptance receipt is required.");
    }
    ...
    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.52 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.52 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.52 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, "v0.52-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.52 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema || obj.GetProperty("Version").GetString() != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.52 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.52 build source manifest.");
        foreach (var item in bound)
        {
            ...
            if (!File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Build-bound v0.52 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.52 acceptance receipt.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.52 checkpoint Git process.");
        ...
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.52 checkpoint Git operation failed: {error.Trim()}");
