...
/// <summary>
/// v0.51.9 local source checkpoint. Runtime owner/lease/index/evidence state is excluded.
/// Exact predecessor identity is dual-bound by source manifest and local accepted tag.
/// </summary>
public sealed class LocalCheckpointV0519Service
{
    public const string Version = "0.51.9";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.51.9";
    public const string ExpectedPredecessorTag = "workbench-v0.51.8-accepted";
    public const string TargetTag = "workbench-v0.51.9-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.51.9 MCP owner generation continuity";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.51.8-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.51.9 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.51.9 local acceptance.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.51.9 tag/version contract.");
        var root = candidate.RepositoryRoot;
        var head = RequireSha((await GitAsync(root, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.51.9 checkpoint preview.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted local v0.51.8 predecessor tag moved after preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.51.9 checkpoint preview.");
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
                     ?? throw new InvalidDataException("v0.51.9 validation artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            var parent = RequireSha((await GitAsync(root, cancellationToken, "rev-parse", "HEAD^")).Stdout, "new HEAD parent");
            if (!parent.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.51.9 checkpoint parent is not exact local v0.51.8 predecessor.");
            await GitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.51.9: MCP owner generation continuity");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.51.9 checkpoint.");

            var nonEffects = new[]
            {
                "exact predecessor is build-manifest-bound and independently matched to workbench-v0.51.8-accepted at HEAD",
                "no remote publication during local checkpoint",
                "no canonical lease, active-index, cross-process fence, owner handle/metadata, generation evidence, stale evidence, adapter or tunnel runtime state committed",
                "no bearer plaintext/hash, endpoint path token, clipboard contents or private application bytes committed",
                "no historical evidence deletion/compaction committed",
                "no external Tools/OpenAI binary committed",
                "no automatic retry/Publish/Lifecycle/catalog/Agent Execute/ActionPermit authority"
            };
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.51.9",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.51.9 validation + manifest/tag-bound predecessor required",
                false, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.51.9-message>", "git tag -a workbench-v0.51.9-accepted" },
                nonEffects);
            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.51.9",
                ...
                $"Local v0.51.9 checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}. Exact predecessor identity was independently matched to {ExpectedPredecessorTag}.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"checkpoint-v0.51.9-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v0519-predecessor", ExpectedPredecessorTag == "workbench-v0.51.8-accepted", ExpectedPredecessorTag, "workbench-v0.51.8-accepted"),
        ("checkpoint-v0519-target", TargetTag == "workbench-v0.51.9-accepted", TargetTag, "workbench-v0.51.9-accepted"),
        ("checkpoint-v0519-manifest", BuildManifestSchema == "matawaka.workbench-build-source-manifest/v0.51", BuildManifestSchema, "v0.51"),
        ("checkpoint-v0519-bootstrap", true, "AcceptFromBootstrapAsync only", "one-shot"),
        ("checkpoint-v0519-runtime-exclusion", true, ".workbench/artifacts/Tools forbidden from changed set", "source only"),
        ("checkpoint-v0519-publication", true, "no remote mutation", "deferred")
    };

    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt receipt)
    {
        if (!receipt.Passed || receipt.Version != Version || receipt.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.51.9 validation receipt is required.");
    }
    ...
    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.51.9 validation artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
                     ?? throw new InvalidDataException("v0.51.9 validation artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Validation artifact does not match passing in-memory v0.51.9 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, "v0.51.9-source-manifest*.json").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.51.9 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema || obj.GetProperty("Version").GetString() != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.51.9 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.51.9 build source manifest.");
        ...
        foreach (var item in bound)
        {
            ...
            if (!full.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Build-bound v0.51.9 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.51.9 validation receipt.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.51.9 checkpoint Git process.");
        ...
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.51.9 checkpoint Git operation failed: {error.Trim()}");
