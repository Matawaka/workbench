...
public sealed record LocalCheckpointCandidateV054(
    string Version,
...
public sealed record LocalCheckpointAuthorityV054(
    string Schema,
    string Subject,
    string Operation,
...
public sealed record LocalCheckpointReceiptV054(
    string Schema,
    string Version,
    ...
    string AppExecutableSha256,
    IReadOnlyList<string> ChangedFiles,
    LocalCheckpointAuthorityV054 Authority,
    bool WorkingTreeCleanAfterCommit,
    IReadOnlyList<string> NonEffects,
    string Note);

public sealed class LocalCheckpointV054Service
{
    public const string Version = "0.54";
    public const string AcceptanceSchema = "matawaka.workbench-acceptance-receipt/v0.54";
    public const string ExpectedPredecessorTag = "workbench-v0.53.2-accepted";
    public const string ExpectedPredecessorCommit = "75f083f7f8383832ff85ecc61dd15ce47ba8d2c9";
    public const string TargetTag = "workbench-v0.54-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.54 bounded runtime-tree materialization lease";
    public const string BuildManifestSchema = "matawaka.workbench-build-source-manifest/v0.54";
    public const string BuildManifestPattern = "v0.54-source-manifest*.json";

    private static readonly TimeSpan GitTimeout = TimeSpan.FromSeconds(30);
    private static readonly JsonSerializerOptions JsonOptions = new() { PropertyNameCaseInsensitive = true, WriteIndented = true };

    public async Task<LocalCheckpointCandidateV054> PreviewAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.54 exact local predecessor mismatch: expected {ExpectedPredecessorCommit}, observed {head}.");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("workbench-v0.53.2-accepted is not at exact current HEAD.");
        ...
        if (changed.Count == 0) throw new InvalidDataException("There are no Workbench v0.54 source changes to checkpoint.");
        ...
        return new LocalCheckpointCandidateV054(
    ...
    public Task<LocalCheckpointReceiptV054> AcceptFromBootstrapAsync(
        LocalCheckpointCandidateV054 candidate,
        string bootstrapLeaseId,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(bootstrapLeaseId))
            throw new InvalidDataException("A claimed transition-bootstrap lease id is required for automatic v0.54 local acceptance.");
        return AcceptCoreAsync(candidate, bootstrapLeaseId, cancellationToken);
    }

    private async Task<LocalCheckpointReceiptV054> AcceptCoreAsync(
        LocalCheckpointCandidateV054 candidate,
        string bootstrapLeaseId,
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.ExpectedPredecessorTag != ExpectedPredecessorTag || candidate.TargetTag != TargetTag)
            throw new InvalidDataException("Checkpoint candidate does not match fixed v0.54 contract.");
        ...
        if (!head.Equals(candidate.PreviousHead, StringComparison.OrdinalIgnoreCase) || !head.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.54 checkpoint preview.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted local v0.53.2 predecessor tag moved after preview.");
        ...
        if (!status.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.54 checkpoint preview.");
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(candidate.AcceptanceArtifactPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.54 acceptance artifact disappeared before checkpoint.");
        ...
        try
        {
            ...
            if (!newParent.Equals(ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("v0.54 checkpoint parent is not the exact local v0.53.2 accepted predecessor.");
            await GitAsync(root, cancellationToken, "tag", "-a", TargetTag, "-m", "Accepted Workbench v0.54: bounded runtime-tree materialization lease");
            tagged = true;
            if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
                throw new InvalidDataException("Workbench working tree is not clean after v0.54 checkpoint.");

            var nonEffects = NonEffects();
            var authority = new LocalCheckpointAuthorityV054(
                "matawaka.workbench-local-checkpoint-bootstrap-authority-receipt/v0.54",
                "human-operator-via-one-shot-transition-bootstrap",
                "git.local-checkpoint.accept",
                root,
                $"one-shot transition-bootstrap lease {bootstrapLeaseId}; exact v0.54 validation + local predecessor binding required",
                false, true, false, false, false, false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.54-message>", "git tag -a workbench-v0.54-accepted" },
                nonEffects);
            return new LocalCheckpointReceiptV054(
                "matawaka.workbench-local-checkpoint-receipt/v0.54",
                ...
                $"Local v0.54 bounded-materialization checkpoint consumed one-shot transition-bootstrap lease {bootstrapLeaseId}; exact predecessor was {ExpectedPredecessorCommit} / {ExpectedPredecessorTag}.");
        }
        catch
        {
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, LocalCheckpointReceiptV054 receipt, CancellationToken cancellationToken)
    {
        var dir = Path.Combine(ResolveRepositoryRoot(workspaceRoot), "artifacts", "acceptance");
        Directory.CreateDirectory(dir);
        var path = Path.Combine(dir, $"checkpoint-v0.54-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("checkpoint-v054-version", Version == "0.54", Version, "0.54"),
        ("checkpoint-v054-exact-predecessor", ExpectedPredecessorCommit == "75f083f7f8383832ff85ecc61dd15ce47ba8d2c9", ExpectedPredecessorCommit, "exact accepted/public v0.53.2"),
        ("checkpoint-v054-predecessor-tag", ExpectedPredecessorTag == "workbench-v0.53.2-accepted", ExpectedPredecessorTag, "workbench-v0.53.2-accepted"),
        ("checkpoint-v054-target-tag", TargetTag == "workbench-v0.54-accepted", TargetTag, "workbench-v0.54-accepted"),
        ("checkpoint-v054-build-manifest", BuildManifestSchema == "matawaka.workbench-build-source-manifest/v0.54" && BuildManifestPattern == "v0.54-source-manifest*.json", BuildManifestSchema, "writer/reader exact target 0.54 binding"),
        ("checkpoint-v054-bootstrap-only", true, "AcceptFromBootstrapAsync only", "no manual visible Accept path"),
        ("checkpoint-v054-publication", true, "RemotePushAllowed=false", "separate post-realhost explicit decision")
    };
    ...
    private static IReadOnlyList<string> NonEffects() => new[]
    {
        ...
        "runtime materialization lease/state/receipt files under artifacts are not committed",
        "v0.52 acquisition and v0.53 runtime execution primitive source semantics are not widened",
        "no runtime process start/stop, network acquisition, benchmark/model/game authority",
        "no private Apps/AppSources or external runtime bytes committed",
    ...
    private static void RequirePassingAcceptance(WorkbenchAcceptanceReceipt acceptance)
    {
        if (!acceptance.Passed || acceptance.Version != Version || acceptance.Schema != AcceptanceSchema)
            throw new InvalidDataException("A passing exact Workbench v0.54 acceptance receipt is required.");
    }

    private static string ValidateAcceptanceArtifact(string root, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath)) throw new InvalidDataException("Passing v0.54 acceptance artifact is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.54 acceptance artifact could not be parsed.");
        if (!parsed.Passed || parsed.Schema != AcceptanceSchema || parsed.Version != Version || parsed.RunId != acceptance.RunId ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.54 receipt.");
        return full;
    }
    ...
    private static (string Path, string Sha256) ValidateBuildSourceManifest(string root, string predecessor, IReadOnlyList<string> changed)
    {
        ...
        var path = Directory.Exists(dir)
            ? Directory.GetFiles(dir, BuildManifestPattern).OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
            : null;
        if (path is null) throw new InvalidDataException("v0.54 build source manifest is missing.");
        ...
        if (obj.GetProperty("Schema").GetString() != BuildManifestSchema || obj.GetProperty("Version").GetString() != Version ||
            ...
            throw new InvalidDataException("Unexpected v0.54 build source manifest identity/predecessor.");
        ...
        if (!bound.Select(x => x.Path).SequenceEqual(current, StringComparer.Ordinal))
            throw new InvalidDataException("Changed-file set differs from v0.54 build source manifest.");
        foreach (var item in bound)
        {
            ...
            if (!File.Exists(full) || !HashFile(full).Equals(item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Build-bound v0.54 source file drift: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expected)
    {
        ...
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path) || !HashFile(path).Equals(expected, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest does not match v0.54 acceptance receipt.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.54 checkpoint Git process.");
        ...
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.54 checkpoint Git operation failed: {error.Trim()}");
