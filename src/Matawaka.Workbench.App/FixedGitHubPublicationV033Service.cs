...
/// <summary>
/// v0.33 successor of the accepted v0.32 fixed GitHub publisher. It preserves
/// the same single-repository, fast-forward-only, non-force authority boundary
/// and changes only the expected locally accepted tag/version/receipt identity.
/// </summary>
public sealed class FixedGitHubPublicationV033Service
{
    ...
    public const string AcceptedTag = "workbench-v0.33-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.33";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.33";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Publication candidate does not match the fixed v0.33 publication contract.");
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted button + separate confirmation dialog after local v0.33 acceptance",
            true,
            true,
            true,
            false,
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.33-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact parent",
                "git push github-workbench refs/tags/workbench-v0.33-accepted:refs/tags/workbench-v0.33-accepted when remote tag absent"
            },
            nonEffects);
        ...
        return new FixedGitHubPublicationReceipt(
            ...
            authority,
            nonEffects,
            "Explicit human maintenance publication of one already-accepted Workbench v0.33 checkpoint. This is not Agent Execute, general network authority, catalog mutation, remote rewrite authority, or canonical UU-AAP publication authority.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"fixed-github-publication-v0.33-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    public static IReadOnlyList<(string Id, bool Passed, string Observed, string Expected)> RunOfflineContractChecks()
    {
        return new[]
        {
            ("publisher-v033-fixed-remote-name", RemoteName == "github-workbench", RemoteName, "github-workbench"),
            ("publisher-v033-fixed-remote-url", RemoteUrl == "https://github.com/Matawaka/workbench.git", RemoteUrl, "fixed Matawaka/workbench URL"),
            ("publisher-v033-fixed-tag", AcceptedTag == "workbench-v0.33-accepted", AcceptedTag, "workbench-v0.33-accepted"),
            ("publisher-v033-parent-fast-forward", ClassifyRemoteMain("a", "b", "a") == "PARENT", ClassifyRemoteMain("a", "b", "a"), "PARENT"),
            ("publisher-v033-idempotent-main", ClassifyRemoteMain("a", "b", "b") == "ALREADY_HEAD", ClassifyRemoteMain("a", "b", "b"), "ALREADY_HEAD"),
            ("publisher-v033-conflicting-main-refused", ClassifyRemoteMain("a", "b", "c") == "CONFLICT", ClassifyRemoteMain("a", "b", "c"), "CONFLICT"),
            ("publisher-v033-absent-tag-admitted", ClassifyRemoteTag("b", null) == "ABSENT", ClassifyRemoteTag("b", null), "ABSENT"),
            ("publisher-v033-idempotent-tag", ClassifyRemoteTag("b", "b") == "ALREADY_HEAD", ClassifyRemoteTag("b", "b"), "ALREADY_HEAD"),
            ("publisher-v033-conflicting-tag-refused", ClassifyRemoteTag("b", "c") == "CONFLICT", ClassifyRemoteTag("b", "c"), "CONFLICT")
        };
    }
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(head, candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Local accepted Workbench v0.33 frontier changed after publication preview.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.33 git publication process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"Fixed v0.33 git publication operation exceeded {GitTimeout.TotalSeconds:0}s timeout: {string.Join(' ', args)}");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"Fixed v0.33 git publication operation failed ({string.Join(' ', args)}): {stderr.Trim()}");
