...
public sealed class FixedGitHubPublicationV0502Service
{
    ...
    public const string AcceptedTag = "workbench-v0.50.2-accepted";
    public const string ExpectedParent = "1b3c5aa44e2bb302764b044b3b3ac00de14a5994";
    public const string ExpectedParentTag = "workbench-v0.50.1-accepted";
    ...
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.50.2";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.50.2";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.50.2 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.50.2 accepted HEAD parent is not exact operator-bound failed-v0.50.1 predecessor: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Local v0.50.1 predecessor tag does not point at exact v0.50.2 parent.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.50.2 tag does not point at current HEAD.");
    ...
    public async Task<FixedGitHubPublicationReceipt> PublishAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName || !SameRemoteUrl(candidate.RemoteUrl, RemoteUrl) || candidate.AcceptedTag != AcceptedTag || !candidate.Parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Publication candidate does not match fixed v0.50.2 stabilization contract.");
        ...
        if (!string.Equals(remoteBaseTag, ExpectedRemoteBase, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted remote v0.49.1 base tag moved; v0.50.2 publication refused.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact accepted v0.49.1 base nor exact v0.50.2 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.50.2 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main disappeared during v0.50.2 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Remote main did not converge to exact accepted v0.50.2 HEAD.");
        if (tagState == "ABSENT")
        {
            await GitAsync(root, cancellationToken, "push", RemoteName, $"refs/tags/{AcceptedTag}:refs/tags/{AcceptedTag}");
            tagPush = true;
        }
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main missing after v0.50.2 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag) ?? throw new InvalidDataException("Remote v0.50.2 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !remoteTagAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.50.2 HEAD.");
        await RequireFailedTagsAbsentAsync(root, cancellationToken);
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout)) throw new InvalidDataException("Local Workbench state changed during v0.50.2 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "failed real-host workbench-v0.50-accepted and workbench-v0.50.1-accepted tags are not published",
            "historical failed workbench-v0.49-accepted tag remains absent remotely",
            "failed v0.50/v0.50.1 commits may exist only as untagged historical ancestry in the fast-forward chain",
            "no arbitrary remote URL/name",
            "no external Tools/OpenAI/tunnel-client binary published",
            "no runtime credential/tunnel session/local endpoint/compatibility facade/private app bytes published",
            "no tunnel process, MCP adapter, compatibility facade or read lease created by publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.50.2 acceptance and successful real-host tunnel readiness + ChatGPT read round-trip",
            ...
            new[]
            {
                ...
                "git push github-workbench <exact-v0.50.2-head>:refs/heads/main when remote main == exact accepted v0.49.1 base",
                "git push github-workbench refs/tags/workbench-v0.50.2-accepted:refs/tags/workbench-v0.50.2-accepted when target tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit publication of accepted Workbench v0.50.2 stabilization. Failed v0.50/v0.50.1 remain only untagged historical ancestry remotely and are not reclassified as accepted frontiers.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.50.2-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v0502-fixed-tag", AcceptedTag == "workbench-v0.50.2-accepted", AcceptedTag, "workbench-v0.50.2-accepted"),
        ("publisher-v0502-local-parent", ExpectedParent == "1b3c5aa44e2bb302764b044b3b3ac00de14a5994", ExpectedParent, "operator-bound local failed-v0.50.1"),
        ("publisher-v0502-remote-base", ExpectedRemoteBase == "1e0453ccf047bd948e76b577c2395a7d0009ff7a", ExpectedRemoteBase, "accepted remote v0.49.1"),
        ("publisher-v0502-failed-tags-suppressed", FailedRemoteTagV0501 == "workbench-v0.50.1-accepted" && FailedRemoteTagV050 == "workbench-v0.50-accepted", $"{FailedRemoteTagV050}/{FailedRemoteTagV0501}", "both absent remotely"),
        ("publisher-v0502-conflict-refused", ClassifyRemoteMain("b", "c") == "CONFLICT" && ClassifyRemoteTag("b", "c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT")
    };
    ...
    {
        if (await ReadRemoteTagCommitAsync(root, ct, FailedRemoteTagV0501) is not null) throw new InvalidDataException("Failed real-host v0.50.1 tag unexpectedly exists remotely; v0.50.2 publication refused.");
        if (await ReadRemoteTagCommitAsync(root, ct, FailedRemoteTagV050) is not null) throw new InvalidDataException("Failed real-host v0.50 tag unexpectedly exists remotely; v0.50.2 publication refused.");
        if (await ReadRemoteTagCommitAsync(root, ct, HistoricalFailedRemoteTagV049) is not null) throw new InvalidDataException("Historical failed v0.49 tag unexpectedly exists remotely; v0.50.2 publication refused.");
    }
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) || !head.Equals(c.Head, StringComparison.OrdinalIgnoreCase) || !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) || !tag.Equals(c.Head, StringComparison.OrdinalIgnoreCase) || !parentTag.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.50.2 frontier changed after publication preview.");
    ...
    private static async Task RequireAncestorAsync(string root, string ancestor, string descendant, CancellationToken ct)
    {
        ...
        if (result.ExitCode != 0) throw new InvalidDataException("Accepted remote v0.49.1 base is not an ancestor of exact local v0.50.2 HEAD.");
    }
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.50.2 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.50.2 publication Git operation timed out.");
        }
        var o = await stdout;
        var e = await stderr;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.50.2 publication Git operation failed: {e.Trim()}");
