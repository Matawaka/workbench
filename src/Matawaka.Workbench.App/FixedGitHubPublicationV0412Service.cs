...
/// <summary>
/// Fixed publication boundary for v0.41.2. The real-host v0.41.1 UX
/// qualification failed after local acceptance, so v0.41.1 remains a local
/// predecessor only. Publication fast-forwards remote main from accepted v0.41
/// through that local commit to v0.41.2 and publishes only the v0.41.2 tag.
/// </summary>
public sealed class FixedGitHubPublicationV0412Service
{
    ...
    public const string AcceptedTag = "workbench-v0.41.2-accepted";
    public const string LocalPredecessorTag = "workbench-v0.41.1-accepted";
    public const string RemoteBaseTag = "workbench-v0.41-accepted";
    public const string ExpectedLocalParent = "9214a0c39ba4acd73e766bf3eb2fdef86b984b9d";
    public const string ExpectedRemoteBase = "1ae2ccd8a0d789c46b26ac29970e2a1a697d8763";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.41.2";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.41.2";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await RunGitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.41.2 publication.");
        ...
        var grandparent = RequireSha((await RunGitAsync(root, cancellationToken, "rev-parse", "HEAD^^")).Stdout, "HEAD grandparent");
        if (!parent.Equals(ExpectedLocalParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.41.2 accepted HEAD parent is not exact local v0.41.1 predecessor: {parent}");
        if (!grandparent.Equals(ExpectedRemoteBase, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.41.2 ancestry does not descend directly from remote accepted v0.41 base: {grandparent}");
        ...
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.41.2 tag does not point at current HEAD.");
        var predecessorTagHead = RequireSha((await RunGitAsync(root, cancellationToken, "rev-list", "-n", "1", LocalPredecessorTag)).Stdout, LocalPredecessorTag);
        if (!predecessorTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local v0.41.1 accepted tag does not point at exact v0.41.2 parent.");
        var remoteBaseTagHead = RequireSha((await RunGitAsync(root, cancellationToken, "rev-list", "-n", "1", RemoteBaseTag)).Stdout, RemoteBaseTag);
        if (!remoteBaseTagHead.Equals(grandparent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Accepted v0.41 tag does not point at exact remote publication base.");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            !candidate.Parent.Equals(ExpectedLocalParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Publication candidate does not match fixed v0.41.2 contract.");
        ...
        var failedPredecessorRemoteTag = await ReadRemoteTagCommitAsync(root, cancellationToken, LocalPredecessorTag);
        if (failedPredecessorRemoteTag is not null)
            throw new InvalidDataException("Remote unexpectedly contains workbench-v0.41.1-accepted; refusing v0.41.2 repair publication because failed real-host qualification must not be tagged remotely.");

        var mainState = ClassifyRemoteMain(ExpectedRemoteBase, candidate.Head, remoteMainBefore);
        var tagState = ClassifyRemoteTag(candidate.Head, remoteTagBefore);
        if (mainState == "CONFLICT")
            throw new InvalidDataException("Remote main is neither exact accepted v0.41 base nor exact local v0.41.2 HEAD.");
        if (tagState == "CONFLICT")
            throw new InvalidDataException("Remote v0.41.2 accepted tag conflicts with local accepted HEAD.");
        ...
        if (mainState == "BASE")
        {
            // Normal Git fast-forward sends the unpublished v0.41.1 parent object as
            // ancestry, but does not publish its failed accepted tag.
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main disappeared during v0.41.2 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main did not converge to exact accepted v0.41.2 HEAD.");
        if (await ReadRemoteTagCommitAsync(root, cancellationToken, LocalPredecessorTag) is not null)
            throw new InvalidDataException("v0.41.1 accepted tag appeared remotely during v0.41.2 publication.");
        ...
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken)
            ?? throw new InvalidDataException("Remote main missing after v0.41.2 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag)
            ?? throw new InvalidDataException("Remote v0.41.2 tag missing after publication.");
        var failedTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, LocalPredecessorTag);
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            failedTagAfter is not null)
            throw new InvalidDataException("Remote v0.41.2 main/tag convergence failed or failed v0.41.1 tag was published.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.41.2 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "failed real-host v0.41.1 accepted tag is not published",
            "no arbitrary remote URL/name",
            "no JSON search/output mutation authority created by publication",
            "no Local Apps import/copy/move authority created by publication",
            "no transition bootstrap/launch/handoff/predecessor-close authority created by publication",
            "no first-boot Self-test/Accept authority created by publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after local v0.41.2 acceptance and real-host visible-selection qualification",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.41.1-accepted (must remain absent)",
                "git ls-remote github-workbench refs/tags/workbench-v0.41.2-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact accepted v0.41 base",
                "git push github-workbench refs/tags/workbench-v0.41.2-accepted:refs/tags/workbench-v0.41.2-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted v0.41.2 focus-primed visible-selection repair. Remote main fast-forwards from accepted v0.41 through the unpublished v0.41.1 predecessor; only the v0.41.2 accepted tag is published.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.41.2-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v0412-fixed-tag", AcceptedTag == "workbench-v0.41.2-accepted", AcceptedTag, "workbench-v0.41.2-accepted"),
        ("publisher-v0412-local-parent", ExpectedLocalParent == "9214a0c39ba4acd73e766bf3eb2fdef86b984b9d", ExpectedLocalParent, "local v0.41.1 accepted commit"),
        ("publisher-v0412-remote-base", ExpectedRemoteBase == "1ae2ccd8a0d789c46b26ac29970e2a1a697d8763", ExpectedRemoteBase, "remote accepted v0.41"),
        ("publisher-v0412-remote-base-classification", ClassifyRemoteMain("a","b","a") == "BASE", ClassifyRemoteMain("a","b","a"), "BASE"),
        ("publisher-v0412-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v0412-failed-v0411-tag-not-pushed", true, "read-only absence check", "no v0.41.1 tag push"),
        ("publisher-v0412-search-not-authority-source", true, "explicit Publish button only", "separate from search")
    };

    public static string ClassifyRemoteMain(string remoteBase, string head, string remote)
        => remote.Equals(remoteBase, StringComparison.OrdinalIgnoreCase) ? "BASE" :
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate c, CancellationToken ct)
    {
        ...
        var grandparent = RequireSha((await RunGitAsync(c.RepositoryRoot, ct, "rev-parse", "HEAD^^")).Stdout, "grandparent");
        var tag = RequireSha((await RunGitAsync(c.RepositoryRoot, ct, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        var predecessorTag = RequireSha((await RunGitAsync(c.RepositoryRoot, ct, "rev-list", "-n", "1", LocalPredecessorTag)).Stdout, LocalPredecessorTag);
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            !head.Equals(c.Head, StringComparison.OrdinalIgnoreCase) ||
            !parent.Equals(ExpectedLocalParent, StringComparison.OrdinalIgnoreCase) ||
            !grandparent.Equals(ExpectedRemoteBase, StringComparison.OrdinalIgnoreCase) ||
            !tag.Equals(c.Head, StringComparison.OrdinalIgnoreCase) ||
            !predecessorTag.Equals(ExpectedLocalParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.41.2 frontier changed after publication preview.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken ct, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.41.2 publication Git process.");
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.41.2 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.41.2 publication Git operation failed: {stderr.Trim()}");
