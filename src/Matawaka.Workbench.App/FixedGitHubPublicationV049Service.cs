...
public sealed class FixedGitHubPublicationV049Service
{
    ...
    public const string AcceptedTag = "workbench-v0.49-accepted";
    public const string ExpectedParent = "2b0e3127e3c0284ff569720d85889cec3374f053";
    public const string ExpectedParentTag = "workbench-v0.48-accepted";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.49";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.49";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace((await GitAsync(root, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.49 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException($"v0.49 accepted HEAD parent is not exact accepted v0.48: {parent}");
        ...
        if (!parentTagHead.Equals(parent, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.48 predecessor tag does not point at exact v0.49 parent.");
        var tagHead = RequireSha((await GitAsync(root, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        if (!tagHead.Equals(head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Accepted v0.49 tag does not point at current HEAD.");
    ...
    public async Task<FixedGitHubPublicationReceipt> PublishAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        if (candidate.Version != Version || candidate.RemoteName != RemoteName || !SameRemoteUrl(candidate.RemoteUrl, RemoteUrl) || candidate.AcceptedTag != AcceptedTag || !candidate.Parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Publication candidate does not match fixed v0.49 contract.");
        ...
        if (mainState == "CONFLICT") throw new InvalidDataException("Remote main is neither exact v0.48 parent nor exact v0.49 HEAD.");
        if (tagState == "CONFLICT") throw new InvalidDataException("Remote v0.49 accepted tag conflicts with local accepted HEAD.");
        ...
        var mid = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main disappeared during v0.49 publication.");
        if (!mid.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Remote main did not converge to exact accepted v0.49 HEAD.");
        if (tagState == "ABSENT")
        {
            await GitAsync(root, cancellationToken, "push", RemoteName, $"refs/tags/{AcceptedTag}:refs/tags/{AcceptedTag}");
            tagPush = true;
        }
        var remoteMainAfter = await ReadRemoteMainAsync(root, cancellationToken) ?? throw new InvalidDataException("Remote main missing after v0.49 publication.");
        var remoteTagAfter = await ReadRemoteTagCommitAsync(root, cancellationToken, AcceptedTag) ?? throw new InvalidDataException("Remote v0.49 tag missing after publication.");
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !remoteTagAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.49 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout)) throw new InvalidDataException("Local Workbench state changed during v0.49 publication.");

        var nonEffects = new[]
        {
            "no force push or conflicting tag movement",
            "no arbitrary remote URL/name",
            "no Apps/AppSources/private-context/clipboard/lease-state/bearer/endpoint-token bytes published",
            "no MCP adapter startup/stop or lease authority created by publication",
            "no Secure MCP Tunnel creation/account login/public endpoint publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted confirmation after automatic local v0.49 acceptance",
            ...
            new[]
            {
                ...
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact v0.48 parent",
                "git push github-workbench refs/tags/workbench-v0.49-accepted:refs/tags/workbench-v0.49-accepted when remote tag absent"
            },
            nonEffects);
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of accepted Workbench v0.49 source only. Mutable lease/adapter state, bearer/endpoint secrets and private app/source bytes are outside this Git publication.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.49-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v049-fixed-tag", AcceptedTag == "workbench-v0.49-accepted", AcceptedTag, "workbench-v0.49-accepted"),
        ("publisher-v049-fixed-parent", ExpectedParent == "2b0e3127e3c0284ff569720d85889cec3374f053", ExpectedParent, "accepted v0.48"),
        ("publisher-v049-parent-tag", ExpectedParentTag == "workbench-v0.48-accepted", ExpectedParentTag, "workbench-v0.48-accepted"),
        ("publisher-v049-conflict-refused", ClassifyRemoteMain("a","b","c") == "CONFLICT" && ClassifyRemoteTag("b","c") == "CONFLICT", "CONFLICT/CONFLICT", "CONFLICT/CONFLICT"),
        ("publisher-v049-runtime-secrets", true, "outside Workbench Git source", "not published")
    };
    ...
    {
        var status = await GitAsync(candidate.RepositoryRoot, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all");
        var head = RequireSha((await GitAsync(candidate.RepositoryRoot, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        var parent = RequireSha((await GitAsync(candidate.RepositoryRoot, cancellationToken, "rev-parse", "HEAD^")).Stdout, "parent");
        var tag = RequireSha((await GitAsync(candidate.RepositoryRoot, cancellationToken, "rev-list", "-n", "1", AcceptedTag)).Stdout, AcceptedTag);
        var parentTag = RequireSha((await GitAsync(candidate.RepositoryRoot, cancellationToken, "rev-list", "-n", "1", ExpectedParentTag)).Stdout, ExpectedParentTag);
        if (!string.IsNullOrWhiteSpace(status.Stdout) || !head.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase) || !tag.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !parentTag.Equals(parent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted v0.49 frontier changed after publication preview.");
    }
    ...
    {
        var result = await GitAsync(root, cancellationToken, true, "remote", "get-url", RemoteName);
        return result.ExitCode == 0 ? result.Stdout.Trim() : null;
    }

    private static async Task<string?> ReadRemoteMainAsync(string root, CancellationToken cancellationToken)
    {
        var result = await GitAsync(root, cancellationToken, "ls-remote", RemoteName, "refs/heads/main");
    ...
    private static async Task<GitResult> GitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.49 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.49 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure) throw new InvalidDataException($"v0.49 publication Git operation failed: {stderr.Trim()}");
