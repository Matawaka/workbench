...
/// <summary>
/// v0.34 local acceptance successor over exact accepted v0.33.
/// It consumes only a passing v0.34 Self-test receipt and the exact v0.34
/// build-source manifest. Remote publication and lifecycle summary remain
/// separate later decisions.
/// </summary>
public sealed class LocalCheckpointV034Service
{
    public const string Version = "0.34.0";
    public const string ExpectedPredecessorCommit = "df211d1f4d80d0b1f238f1166460758e73ce18d2";
    public const string ExpectedPredecessorTag = "workbench-v0.33-accepted";
    public const string TargetTag = "workbench-v0.34-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.34 accepted maintenance lifecycle receipt boundary";
        ...
        CancellationToken cancellationToken)
    {
        if (!acceptance.Passed ||
            ...
            !string.Equals(acceptance.Schema, "matawaka.workbench-acceptance-receipt/v0.34", StringComparison.Ordinal))
            throw new InvalidDataException("A passing Workbench v0.34 Self-test receipt is required before local checkpoint acceptance.");
        ...
        var currentHead = RequireSha((await RunGitAsync(repositoryRoot, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!string.Equals(currentHead, ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"HEAD {currentHead} is not exact accepted v0.33 predecessor {ExpectedPredecessorCommit}.");
        var predecessorTagHead = RequireSha((await RunGitAsync(repositoryRoot, cancellationToken, "rev-list", "-n", "1", ExpectedPredecessorTag)).Stdout, ExpectedPredecessorTag);
        if (!string.Equals(predecessorTagHead, ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"Accepted predecessor tag {ExpectedPredecessorTag} no longer points at exact v0.33 predecessor.");
        ...
        var changedFiles = ParseStatusPaths((await RunGitAsync(repositoryRoot, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout);
        if (changedFiles.Count == 0)
            throw new InvalidDataException("There are no Workbench v0.34 source changes to checkpoint.");
        if (changedFiles.Any(IsForbiddenCheckpointPath))
            throw new InvalidDataException("Checkpoint candidate contains a forbidden artifacts/.workbench path.");
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match v0.34 fixed acceptance contract.");

        var repositoryRoot = candidate.RepositoryRoot;
        var headBefore = RequireSha((await RunGitAsync(repositoryRoot, cancellationToken, "rev-parse", "HEAD")).Stdout, "HEAD");
        if (!string.Equals(headBefore, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.34 checkpoint preview.");
        ...
        if (!currentStatus.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.34 checkpoint preview.");
        ...
        try
        {
            ...
            var newHead = RequireSha((await RunGitAsync(repositoryRoot, cancellationToken, "rev-parse", "HEAD")).Stdout, "new HEAD");
            if (string.Equals(newHead, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Git commit did not advance v0.34 HEAD.");

            await RunGitAsync(repositoryRoot, cancellationToken,
                "tag", "-a", TargetTag,
                "-m", "Accepted Workbench v0.34: lifecycle receipt binds exact update/build, Self-test, checkpoint and publication evidence without creating lifecycle authority, automatic transition, retry, rollback, Agent Execute or canonical UU-AAP conformance");
            tagCreated = true;

            var cleanStatus = (await RunGitAsync(repositoryRoot, cancellationToken, "status", "--porcelain=v1", "--untracked-files=all")).Stdout;
            if (!string.IsNullOrWhiteSpace(cleanStatus))
                throw new InvalidDataException("Workbench working tree is not clean after v0.34 checkpoint commit.");
            ...
            var nonEffects = new[]
            {
                ...
                "no lifecycle receipt authority inferred from checkpoint",
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.34",
                "human-operator-at-workbench-ui",
                "git.local-checkpoint.accept",
                repositoryRoot,
                "explicit v0.34 Accept checkpoint button + confirmation dialog after passing v0.34 Self-test",
                true,
                true,
                false,
                false,
                false,
                false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.34-message>", "git tag -a workbench-v0.34-accepted" },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.34",
                Version,
                DateTimeOffset.Now,
                ...
                true,
                nonEffects,
                "Local Workbench v0.34 acceptance only. Remote publication and lifecycle receipt remain separate later evidence/authority decisions and are not implied by this checkpoint.");
        }
        catch
        {
            if (tagCreated)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.34-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
        await File.WriteAllTextAsync(path, JsonSerializer.Serialize(receipt, JsonOptions), new UTF8Encoding(false), cancellationToken);
        return path;
    }

    private static string ValidateAcceptanceArtifact(string repositoryRoot, string artifactPath, WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.34 acceptance artifact file is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.34 acceptance artifact could not be parsed.");
        if (!parsed.Passed ||
            !string.Equals(parsed.Schema, "matawaka.workbench-acceptance-receipt/v0.34", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.34 Self-test receipt.");
        return full;
    }
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(checkpointDir, "v0.34.0-source-manifest*.json", SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.34 build source manifest is missing. Refusing to checkpoint source that is not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.34 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.34", StringComparison.Ordinal) ||
            ...
            !string.Equals(predecessorHead, ExpectedPredecessorCommit, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Unexpected v0.34 build source manifest identity/predecessor.");
        ...
        if (!manifestFiles.Select(item => item.Path).SequenceEqual(currentFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Current Workbench changed-file set differs from the v0.34 source set captured at build time.");
        foreach (var item in manifestFiles)
        {
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full))
                throw new InvalidDataException($"Build-bound v0.34 source file missing or escapes Workbench root: {item.Path}");
            if (!string.Equals(HashFile(full), item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"v0.34 source file changed after candidate build: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expectedSha256)
    {
        ...
        if (!string.Equals(HashFile(path), expectedSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest no longer matches v0.34 Self-test receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.34 checkpoint git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.34 checkpoint git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.34 checkpoint git operation failed: {stderr.Trim()}");
