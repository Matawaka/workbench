...
/// <summary>
/// v0.32 successor checkpoint gate. It intentionally leaves the historical
/// LocalCheckpointService v0.31 source unchanged and creates a separate bounded
/// acceptance path for the new candidate.
/// </summary>
public sealed class LocalCheckpointV032Service
{
    public const string Version = "0.32.0";
    public const string ExpectedPredecessorTag = "workbench-v0.31-accepted";
    public const string TargetTag = "workbench-v0.32-accepted";
    public const string CommitMessage = "Checkpoint Workbench v0.32 accepted maintenance integrator and fixed source publication boundary";
    ...
    {
        if (!acceptance.Passed || !string.Equals(acceptance.Version, Version, StringComparison.Ordinal) ||
            !string.Equals(acceptance.Schema, "matawaka.workbench-acceptance-receipt/v0.32", StringComparison.Ordinal))
            throw new InvalidDataException("A passing Workbench v0.32 Self-test receipt is required before local checkpoint acceptance.");
        ...
        if (changedFiles.Count == 0)
            throw new InvalidDataException("There are no Workbench v0.32 source changes to checkpoint.");
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(candidate.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Checkpoint candidate does not match v0.32 fixed acceptance contract.");
        ...
        if (!string.Equals(headBefore, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Workbench HEAD changed after v0.32 checkpoint preview.");
        ...
        if (!currentStatus.SequenceEqual(candidate.ChangedFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Workbench working tree changed after v0.32 checkpoint preview.");
        ...
        try
        {
            ...
            if (string.Equals(newHead, candidate.PreviousHead, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Git commit did not advance v0.32 HEAD.");

            await RunGitAsync(repositoryRoot, cancellationToken,
                "tag", "-a", TargetTag,
                "-m", "Accepted Workbench v0.32: maintenance surface consolidated; fixed Matawaka/workbench publication remains explicit, fast-forward-only, non-force, tag-conflict-refusing, separate from Agent Execute and general network authority");
            tagCreated = true;
            ...
            if (!string.IsNullOrWhiteSpace(cleanStatus))
                throw new InvalidDataException("Workbench working tree is not clean after v0.32 checkpoint commit.");
            ...
            var authority = new LocalCheckpointAuthorityReceipt(
                "matawaka.workbench-local-checkpoint-authority-receipt/v0.32",
                "human-operator-at-workbench-ui",
                "git.local-checkpoint.accept",
                repositoryRoot,
                "explicit v0.32 Accept checkpoint button + confirmation dialog after passing v0.32 Self-test",
                true,
                true,
                false,
                false,
                false,
                false,
                new[] { "git add -A -- .", "git commit -m <fixed-v0.32-message>", "git tag -a workbench-v0.32-accepted" },
                nonEffects);

            return new LocalCheckpointReceipt(
                "matawaka.workbench-local-checkpoint-receipt/v0.32",
                Version,
                DateTimeOffset.Now,
                ...
                true,
                nonEffects,
                "Local Workbench v0.32 acceptance only. Remote publication remains a separate later Publish accepted decision and is not implied by this checkpoint.");
        }
        catch
        {
            if (tagCreated)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(directory, $"checkpoint-v0.32-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
        ...
        WorkbenchAcceptanceReceipt acceptance)
    {
        if (string.IsNullOrWhiteSpace(artifactPath) || !File.Exists(artifactPath))
            throw new InvalidDataException("Passing v0.32 acceptance artifact file is missing.");
        ...
        var parsed = JsonSerializer.Deserialize<WorkbenchAcceptanceReceipt>(File.ReadAllText(full, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.32 acceptance artifact could not be parsed.");
        if (!parsed.Passed || !string.Equals(parsed.Version, Version, StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Acceptance artifact does not match the passing in-memory v0.32 Self-test receipt.");
        return full;
    }
        ...
        IReadOnlyList<string> changedFiles)
    {
        ...
        var manifestPath = Directory.Exists(checkpointDir)
            ? Directory.GetFiles(checkpointDir, "v0.32.0-source-manifest*.json", SearchOption.TopDirectoryOnly)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault()
            : null;
        if (string.IsNullOrWhiteSpace(manifestPath))
            throw new InvalidDataException("v0.32 build source manifest is missing. Refusing to checkpoint source that is not byte-bound to the built candidate.");

        var manifest = JsonSerializer.Deserialize<BuildSourceManifest>(File.ReadAllText(manifestPath, Encoding.UTF8), JsonOptions)
            ?? throw new InvalidDataException("v0.32 build source manifest could not be parsed.");
        if (!string.Equals(manifest.Schema, "matawaka.workbench-build-source-manifest/v0.32", StringComparison.Ordinal) ||
            ...
            throw new InvalidDataException("Unexpected v0.32 build source manifest identity/predecessor.");
        ...
        if (!manifestFiles.Select(item => item.Path).SequenceEqual(currentFiles, StringComparer.Ordinal))
            throw new InvalidDataException("Current Workbench changed-file set differs from the v0.32 source set captured at build time.");
        ...
        foreach (var item in manifestFiles)
        {
            ...
            if (!full.StartsWith(rootPrefix, StringComparison.OrdinalIgnoreCase) || !File.Exists(full))
                throw new InvalidDataException($"Build-bound v0.32 source file missing or escapes Workbench root: {item.Path}");
            if (!string.Equals(HashFile(full), item.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"v0.32 source file changed after candidate build: {item.Path}");
        }
    ...
    private static void VerifyRunningExecutable(string expectedSha256)
    {
        ...
        if (!string.Equals(HashFile(path), expectedSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Running executable digest no longer matches v0.32 Self-test receipt.");
    }
        ...
        params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.32 checkpoint git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException($"v0.32 checkpoint git operation exceeded {GitTimeout.TotalSeconds:0}s timeout.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.32 checkpoint git operation failed: {stderr.Trim()}");
