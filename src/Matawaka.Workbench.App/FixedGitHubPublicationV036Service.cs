...
public sealed class FixedGitHubPublicationV036Service
{
    ...
    public const string AcceptedTag = "workbench-v0.36-accepted";
    public const string ExpectedParent = "bf4c59e251c4e9642cc0ca7085e5c3af24622707";
    public const string ReceiptSchema = "matawaka.workbench-fixed-github-publication-receipt/v0.36";
    public const string AuthoritySchema = "matawaka.workbench-fixed-github-publication-authority-receipt/v0.36";
    ...
    public async Task<FixedGitHubPublicationCandidate> PreviewAsync(string workspaceRoot, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout))
            throw new InvalidDataException("Workbench working tree must be clean before v0.36 publication.");
        ...
        if (!parent.Equals(ExpectedParent, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"v0.36 accepted HEAD parent is not exact accepted v0.35.1: {parent}");
        ...
        CancellationToken cancellationToken)
    {
        if (candidate is null || candidate.Version != Version || candidate.RemoteName != RemoteName ||
            ...
            throw new InvalidDataException("Publication candidate does not match fixed v0.36 contract.");
        ...
        if (mainState == "CONFLICT")
            throw new InvalidDataException("Remote main is neither exact local parent nor exact local HEAD.");
        var tagState = ClassifyRemoteTag(candidate.Head, remoteTagBefore);
        if (tagState == "CONFLICT")
            throw new InvalidDataException("Remote accepted tag conflicts with local accepted HEAD.");
        ...
        if (!remoteMainAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidDataException("Remote main/tag do not equal exact accepted v0.36 HEAD.");
        ...
        if (!headAfter.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase) || !string.IsNullOrWhiteSpace(statusAfter.Stdout))
            throw new InvalidDataException("Local Workbench state changed during v0.36 publication.");
        ...
        var nonEffects = new[]
        {
            ...
            "no local-app registration/update authority created by publication",
        ...
        var authority = new FixedGitHubPublicationAuthorityReceipt(
            ...
            RemoteName,
            RemoteUrl,
            "explicit Publish accepted button + separate confirmation dialog after local v0.36 acceptance",
            ...
            new[]
            {
                ...
                "git ls-remote github-workbench refs/tags/workbench-v0.36-accepted",
                "git push github-workbench <exact-head>:refs/heads/main when remote main == exact parent",
                "git push github-workbench refs/tags/workbench-v0.36-accepted:refs/tags/workbench-v0.36-accepted when remote tag absent"
            },
            nonEffects);
        ...
        return new FixedGitHubPublicationReceipt(
            ...
            "Explicit human publication of one already-accepted Workbench v0.36 checkpoint. This does not create local-app registration/update, lifecycle, Agent Execute or general network authority.");
    }
    ...
    public static async Task<string> WriteReceiptAsync(string workspaceRoot, FixedGitHubPublicationReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(dir, $"fixed-github-publication-v0.36-{DateTime.Now:yyyyMMdd-HHmmssfff}.json");
    ...
    {
        ("publisher-v036-fixed-tag", AcceptedTag == "workbench-v0.36-accepted", AcceptedTag, "workbench-v0.36-accepted"),
        ("publisher-v036-fixed-parent", ExpectedParent == "bf4c59e251c4e9642cc0ca7085e5c3af24622707", ExpectedParent, "accepted v0.35.1"),
        ("publisher-v036-conflicting-main-refused", ClassifyRemoteMain("a", "b", "c") == "CONFLICT", ClassifyRemoteMain("a", "b", "c"), "CONFLICT"),
        ("publisher-v036-conflicting-tag-refused", ClassifyRemoteTag("b", "c") == "CONFLICT", ClassifyRemoteTag("b", "c"), "CONFLICT")
    };

    public static string ClassifyRemoteMain(string parent, string head, string remoteMain)
        => remoteMain.Equals(parent, StringComparison.OrdinalIgnoreCase) ? "PARENT" :
           remoteMain.Equals(head, StringComparison.OrdinalIgnoreCase) ? "ALREADY_HEAD" : "CONFLICT";

    public static string ClassifyRemoteTag(string head, string? remoteTag)
        => string.IsNullOrWhiteSpace(remoteTag) ? "ABSENT" :
           remoteTag.Equals(head, StringComparison.OrdinalIgnoreCase) ? "ALREADY_HEAD" : "CONFLICT";
    ...
    private static async Task ReverifyLocalCandidateAsync(FixedGitHubPublicationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status.Stdout) ||
            ...
            !tagHead.Equals(candidate.Head, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Local accepted Workbench v0.36 frontier changed after publication preview.");
    }
    ...
    private static async Task<GitResult> RunGitAsync(string root, CancellationToken cancellationToken, bool allowFailure, params string[] args)
    {
        ...
        if (!process.Start()) throw new InvalidDataException("Failed to start fixed v0.36 publication Git process.");
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new InvalidDataException("v0.36 publication Git operation timed out.");
        }
        var stdout = await stdoutTask;
        var stderr = await stderrTask;
        if (process.ExitCode != 0 && !allowFailure)
            throw new InvalidDataException($"v0.36 publication Git operation failed: {stderr.Trim()}");
