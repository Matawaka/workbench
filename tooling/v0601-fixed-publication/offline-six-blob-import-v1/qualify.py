...
class Fixture:
 def __init__(self,base):
  ...
  self.receipt=self.attempt.with_name('six-blobs-'+self.head+'.json')
  self.script=base/'fixture.ps1'
 def run(self,token=CONFIRM,extra=()):
  # Rich failure diagnostics only in disposable fixture text, not in production delivery.
...
case('wrong-ordered-parents',lambda f:setattr(f,'code',f.code.replace("$first = '"+f.first+"'","$first = '"+f.second+"'")),expected='PREDECESSOR_TAG_MISMATCH')
...
case('post-confirmation-source-drift-before-first-write',lambda f:setattr(f,'code',f.code.replace('$fresh=Snapshot;','[IO.File]::WriteAllText((Join-Path $root "current.txt"),"fixture concurrent drift"); $fresh=Snapshot;')),expected='SNAPSHOT_CHANGED_BEFORE_IMPORT')
