# Diagnostic v2, exact side-by-side Git. Console observation only, not a publication permit.
# Keep historical diagnostic v1 refusal unchanged. No installer, PATH/config changes or hydration.
    ...
    try {
        ...
        function Count-Objects([string]$text) {
            ...
            foreach ($line in ($text -split '\r?\n')) {
                if ($line -cmatch '^\?[0-9a-f]{40}$') { $missing++ }
                elseif ($line -cmatch '^[0-9a-f]{40}$') { $present++ }
        ...
        if ($rootTreePresent) {
            ...
            $tree = [pscustomobject]@{ Present = 0; Missing = 1 }
        }
        ...
        [pscustomobject]@{
            Diagnostic = 'OFFLINE_OBJECT_ENUMERATION_V2_EXACT_SIDE_BY_SIDE_GIT'
            GitVersion = $version
