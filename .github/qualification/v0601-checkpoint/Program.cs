...
internal static class Program
{
    ...
    private static async Task RunParentAsync()
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "matawaka-v0601-checkpoint-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var changedBytes = new UTF8Encoding(false).GetBytes("namespace Matawaka.Workbench.App;\ninternal static class V0601CheckpointQualificationPayload { internal const string Marker = \"exact-v0601-checkpoint\"; }\n");
            await File.WriteAllBytesAsync(changedFull, changedBytes);
            var changedSha = Sha256(changedFull);
            Require(StatusPaths(repository).SequenceEqual(new[] { changedPath }, StringComparer.Ordinal), "sandbox changed set is not exact single-file fixture");
            ...
            await File.WriteAllTextAsync(manifestPath, JsonSerializer.Serialize(manifest, JsonOptions), new UTF8Encoding(false));
            ...
            Require(child.ExitCode == 0, "checkpoint qualification child failed: exit=" + child.ExitCode);
            Require(File.Exists(resultPath), "checkpoint qualification child result missing");
            ...
            Require(r.GetProperty("two_parent_commit_created").GetBoolean(), "two-parent commit not proven");
            Require(r.GetProperty("parent_order_verified").GetBoolean(), "parent order not proven");
            Require(r.GetProperty("one_shot_replay_refused").GetBoolean(), "one-shot replay refusal not proven");
            Require(r.GetProperty("hostile_state_refused").GetBoolean(), "hostile state refusal not proven");
            Require(r.GetProperty("hostile_pid_refused").GetBoolean(), "hostile PID refusal not proven");
            Require(r.GetProperty("hostile_process_sha_refused").GetBoolean(), "hostile process SHA refusal not proven");
            Require(r.GetProperty("hostile_acceptance_tamper_refused").GetBoolean(), "acceptance tamper refusal not proven");
            Require(r.GetProperty("hostile_import_tamper_refused").GetBoolean(), "import tamper refusal not proven");
            Require(r.GetProperty("hostile_source_drift_refused").GetBoolean(), "source drift refusal not proven");
            Require(r.GetProperty("bootstrap_completed_accepted").GetBoolean(), "bootstrap final state not proven");
            Require(!r.GetProperty("remote_push_allowed").GetBoolean(), "checkpoint widened remote push authority");
            Require(!r.GetProperty("network_access_allowed").GetBoolean(), "checkpoint widened network authority");
            Require(!r.GetProperty("automatic_retry_allowed").GetBoolean(), "checkpoint widened retry authority");

            var output = new
            {
                schema = "matawaka.workbench-v0601-checkpoint-sandbox-qualification/v0.1",
    ...
    private static async Task<int> RunChildAsync(string workspace)
    {
        ...
        var parentLine = Git(repository, "rev-list", "--parents", "-n", "1", accepted.NewHead).Trim();
        var parentParts = parentLine.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        ...
        var result = new
        {
            schema = "matawaka.workbench-v0601-checkpoint-child-qualification/v0.1",
    ...
    private static void WriteImportReceipt(string repository)
    {
        ...
        var path = Path.Combine(dir, "public-main-6541dc32182c970c8e1a6ade426a6cee7086511b.json");
        var receipt = new
        {
            ...
            Note = "Sandbox verifier fixture only. The operator-host requires the separately generated canonical no-ref import receipt."
        };
